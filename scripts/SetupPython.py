import sys
import subprocess
import importlib.util as imporlib_util
...
class PythonConfiguration:
    ...
    def __ValidatePython(cls, versionMajor = 3, versionMinor = 3):
        if sys.version is not None:
            ...
            if sys.version_info.major < versionMajor or (sys.version_info.major == versionMajor and
                                                         sys.version_info.minor < versionMinor):
    ...
    def __ValidatePackage(cls, packageName):
        if imporlib_util.find_spec(packageName) is None:
    ...
    def __InstallPackage(cls, packageName):
        ...
        subprocess.check_call(['python', '-m', 'pip', 'install', packageName, '-i', 
