...
ANT_REQUIRED_VULKAN_VERSION = '1.3.' # Any 1.3 version is fine
ANT_INSTALL_VULKAN_VERSION = '1.3.216.0' # Specifically install this one if no 1.3 version is present
VULKAN_SDK_INSTALLER_URL = f'https://sdk.lunarg.com/sdk/download/{ANT_INSTALL_VULKAN_VERSION}/windows/VulkanSDK-{ANT_INSTALL_VULKAN_VERSION}-Installer.exe'
VULKAN_SDK_LOCAL_PATH = 'Ant/vendor/VulkanSDK'
...
def CheckVulkanSDK():
    if (VULKAN_SDK is None):
        ...
        InstallVulkanPrompt()
        return False
    elif (ANT_REQUIRED_VULKAN_VERSION not in VULKAN_SDK):
        print(f"Located Vulkan SDK at {VULKAN_SDK}")
        print(
            f"{Style.BRIGHT}{Back.RED}You don't have the correct Vulkan SDK version! (Hazel requires {ANT_REQUIRED_VULKAN_VERSION}){Style.RESET_ALL}")
        InstallVulkanPrompt()
        return False
