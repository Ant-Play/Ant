...
namespace Ant {
	...
	AudioComponent* AudioComponentRegistry::GetAudioComponent(Ant::UUID sceneID, uint64_t audioComponentID) const
	{
		std::optional<Ant::Entity> o = Get(sceneID, audioComponentID);
		if (o.has_value())
			return &o->GetComponent<AudioComponent>();
		else
		{
			ANT_CORE_ASSERT("Component was not found in registry");
			return nullptr;
		}
	}

	Ant::Entity AudioComponentRegistry::GetEntity(Ant::UUID sceneID, uint64_t audioComponentID) const
	{
		return Get(sceneID, Ant::UUID(audioComponentID)).value_or(Ant::Entity());
	}
	...
	using TriggerScoped = Ant::Scope<TriggerCommand>;

	uint64_t ActiveTriggersRegistry::Count(Ant::UUID sceneID)
	{
		return GetCommandCount(sceneID);
	}

	std::optional<std::vector<TriggerScoped>*> ActiveTriggersRegistry::GetCommands(Ant::UUID sceneID, Ant::UUID objectID) const
	{
		return m_ActiveCommands.Get(sceneID, objectID);
	}

	void ActiveTriggersRegistry::AddTrigger(Ant::UUID sceneID, Ant::UUID objectID, TriggerCommand* trigger)
	{
		if (auto* triggers = m_ActiveCommands.Get(sceneID, objectID).value_or(nullptr))
		{
			triggers->push_back(Ant::Scope<TriggerCommand>(trigger));
		}
		else
		{
			auto newTriggerList = new std::vector<std::unique_ptr<TriggerCommand>>();
			newTriggerList->push_back(Ant::Scope<TriggerCommand>(trigger));
			m_ActiveCommands.Add(sceneID, objectID, newTriggerList);
		}
	}

	bool ActiveTriggersRegistry::RemoveTrigger(Ant::UUID sceneID, Ant::UUID objectID, TriggerCommand* trigger)
	{
		if (auto* triggers = m_ActiveCommands.Get(sceneID, objectID).value_or(nullptr))
		{
	...
	bool ActiveTriggersRegistry::OnTriggerActionHandled(Ant::UUID sceneID, Ant::UUID objectID, TriggerCommand* trigger)
	{
	...
	bool ActiveTriggersRegistry::OnPlaybackActionFinished(Ant::UUID sceneID, Ant::UUID objectID, Ant::Ref<SoundConfig> soundSource)
	{
		if (auto* triggers = m_ActiveCommands.Get(sceneID, objectID).value_or(nullptr))
		{
			...
			auto it = std::find_if(triggers->begin(), triggers->end(), [this, &commandFullyHandled, sceneID, objectID, soundSource](TriggerScoped& trigger)
				{
					...
					{
						ANT_CORE_INFO("removed play trigger");
	...
	bool ActiveTriggersRegistry::RefreshTriggers(Ant::UUID sceneID, Ant::UUID objectID)
	{
		bool commandFullyHandled = false;
		...
		if (auto* objectTriggers = m_ActiveCommands.Get(sceneID, objectID).value_or(nullptr))
		{
			...
			if (itr != objectTriggers->end())
			{
				...
				ANT_CORE_INFO("Deleting finished commands, removed: {0}", num); //? for soem reasong this throws: itr->get()->DebugName
			...
			{
				ANT_CORE_INFO("All Commands on object {0} handled.", objectID);
	...
	*/
	...
	namespace Audio
	{
		...
		EventInfo::EventInfo(const CommandID& comdID, Ant::UUID objectID)
		...
		bool EventRegistry::Remove(EventID eventID)
		{
			...
			ANT_CORE_ASSERT(m_PlaybackInstances.find(eventID) != m_PlaybackInstances.end());
		...
		bool EventRegistry::RemoveSource(EventID eventID, SourceID sourceID)
		{
			...
			if (it == sources.end())
				ANT_CORE_ERROR("Audio. EventRegistry. Attempted to remove source that's not associated to any Event in registry.");
			else
				sources.erase(it);
		...
		uint32_t ObjectEventRegistry::GetTotalPlaybackCount() const
		{
			...
			return (uint32_t)std::accumulate(m_Objects.begin(), m_Objects.end(), 0,
				[](uint32_t prior, const std::pair<Ant::UUID, std::vector<EventID>>& item) -> uint32_t
				{
					return prior + (uint32_t)item.second.size();
				});
		}

		bool ObjectEventRegistry::Add(Ant::UUID objectID, EventID eventID)
		{
			std::scoped_lock lock{ m_Mutex };
			...
			if (m_Objects.find(objectID) != m_Objects.end())
			{
				...
				{
					ANT_CORE_ASSERT(false, "EventID already associated to the object.");
					return false;
				}
		...
		bool ObjectEventRegistry::Remove(Ant::UUID objectID, EventID eventID)
		{
			std::scoped_lock lock{ m_Mutex };

			ANT_CORE_ASSERT(m_Objects.find(objectID) != m_Objects.end());

			auto& events = m_Objects.at(objectID);
			auto it = std::find(events.begin(), events.end(), eventID);

			if (it == events.end())
				ANT_CORE_ERROR("Audio. ObjectEventRegistry. Attempted to remove Event that's not associated to any Object in registry.");
			else
				events.erase(it);
		...
		bool ObjectEventRegistry::RemoveObject(Ant::UUID objectID)
		{
			std::scoped_lock lock{ m_Mutex };
		...
		uint32_t ObjectEventRegistry::GetNumberOfEvents(Ant::UUID objectID) const
		{
			std::shared_lock lock{ m_Mutex };
		...
		std::vector<EventID> ObjectEventRegistry::GetEvents(Ant::UUID objectID) const
		{
			std::shared_lock lock{ m_Mutex };
		...
		uint32_t ObjectSourceRegistry::GetTotalPlaybackCount() const
		{
			...
			return (uint32_t)std::accumulate(m_Objects.begin(), m_Objects.end(), 0,
				[](uint32_t prior, const std::pair<Ant::UUID, std::vector<SourceID>>& item) -> uint32_t
				{
					return prior + (uint32_t)item.second.size();
				});
		}

		bool ObjectSourceRegistry::Add(Ant::UUID objectID, SourceID sourceID)
		{
			std::scoped_lock lock{ m_Mutex };
			...
			if (m_Objects.find(objectID) != m_Objects.end())
			{
				...
				{
					ANT_CORE_ASSERT(false, "Active Sound already associated to the object.");
					return false;
				}
		...
		bool ObjectSourceRegistry::Remove(Ant::UUID objectID, SourceID sourceID)
		{
			std::scoped_lock lock{ m_Mutex };
			...
			if (it == sounds.end())
				ANT_CORE_ERROR("Audio. ObjectEventRegistry. Attempted to remove Source that's not associated to any Object in registry.");
			else
				sounds.erase(it);
		...
		bool ObjectSourceRegistry::RemoveObject(Ant::UUID objectID)
		{
			std::scoped_lock lock{ m_Mutex };
		...
		uint32_t ObjectSourceRegistry::GetNumberOfActiveSounds(Ant::UUID objectID) const
		{
			std::shared_lock lock{ m_Mutex };
		...
		std::optional<std::vector<SourceID>> ObjectSourceRegistry::GetActiveSounds(Ant::UUID objectID) const
		{
			std::shared_lock lock{ m_Mutex };
