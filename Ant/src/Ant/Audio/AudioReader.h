...
namespace Ant::Audio {
	...
	struct DataSourceContext
	{
		struct Reader
		{
			...
			{
				ANT_CORE_ASSERT(destination != nullptr);
				...
				if (leftToRead > 0 && (Source.flags & MA_RESOURCE_MANAGER_DATA_SOURCE_FLAG_DECODE))
				{
					...
					ANT_CORE_ASSERT(framesRead == numFramesToRead);
				}
		...
		bool InitializeReader(ma_resource_manager* resourceManager, AssetHandle waveAssetHandle)
		{
			if (!Application::IsRuntime())
			{
				...
				ANT_CORE_ASSERT(assetMetadata.Type == AssetType::Audio);
			...
			ANT_CORE_ASSERT(sourceFormat == ma_format_f32);
			// For now we only support mono and stereo files
			ANT_CORE_ASSERT(reader.NumChannels > 0 && reader.NumChannels <= 2);
