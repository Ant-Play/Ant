...
#pragma endregion
    ...
    {
        ANT_CORE_ASSERT(numOfInputs > 0 && numOfOutputs > 0);
        ...
        for (int i = 0; i < (int)numOfOutputs; i++)
        {
            ...
            angles[i] = angle;
            //ANT_CORE_TRACE("Angle degrees: {}", glm::degrees(angle));
        ...
        for (int i = 0; i < vbap->spPosSorted.size(); i++)
        {
            ...
            //ANT_CORE_TRACE("Sorted degrees: {}", glm::degrees(VectorAngle({ p.x, 0.0f, p.y })));
        ...
        for (uint32_t i = 0; i < numOfInputs; i++)
        {
            ...
            /*const auto an = g_maChannelDirections[m_SurrChannelMap[i]];
              ...
              ANT_CORE_TRACE("-----------------------------");
              ANT_CORE_TRACE("Channel GRP ID {0}, angle: {1}, original angle {2}", id, channelAngle, originalAngle);*/
            ...
            for (uint32_t vi = 0; vi < vsPerChannel; vi++)
            {
                ...
                /*if (channelAngle < 0.0f)
                        ANT_CORE_TRACE("Left Channel VS: channelAngle {0}, vsAngle {1}", channelAngle, sa);
                    else
                        ANT_CORE_TRACE("Right Channel VS: channelAngle {0}, vsAngle {1}", channelAngle, sa);*/
    ...
    {
        ANT_CORE_ASSERT(!vbap->VirtualSources.empty());
    ...
    void VBAP::ProcessVBAP(const VBAPData* vbap, const float azimuthRadians, ChannelGains& gains)
    {
        ...
        {
            //ANT_CONSOLE_LOG_ERROR("Faild to find Active Arch.");
        }
    }
