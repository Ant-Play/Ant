...
namespace Ant::Audio::DSP {
    ...
    static void spatializer_node_process_pcm_frames(ma_node* pNode, const float** ppFramesIn, ma_uint32* pFrameCountIn, float** ppFramesOut, ma_uint32* pFrameCountOut)
    {
        //===== Audio Buffer data ======
    ...
    {
        ANT_CORE_ASSERT(IsInitialized(sourceID))
    ...
    void Spatializer::SetSpread(uint32_t sourceID, float newSpread)
    {
        ...
        {
            ANT_CORE_ERROR_TAG("Sound Spatializer", "SetSpread({0}, {1}) - invalid sourseID.", sourceID, newSpread);
            return;
        }
        ...
        {
            ANT_CORE_WARN_TAG("Sound Spatializer", "Trying to update sound Spread value while using SpreadFromSourceSize.");
            return;
        }
    ...
    void Spatializer::SetFocus(uint32_t sourceID, float newFocus)
    {
        if (!IsInitialized(sourceID))
        {
            ANT_CORE_ERROR_TAG("Sound Spatializer", "SetFocus({0}, {1}) - invalid sourseID.", sourceID, newFocus);
            return;
        }
    ...
    void Spatializer::UpdatePositionalData(Source& source, const ma_spatializer_listener* listener)
    {
        ...
        if (distance > 0)
        {
            ...
            if (listener != nullptr && listener->config.coneInnerAngleInRadians < 6.283185f)
            {
                ...
                //ANT_CORE_TRACE_TAG("Sound Spatializer", "Angle Attenuation: {}", angleAttenuation);
            }
        ...
        //ANT_CORE_TRACE_TAG("Sound Spatializer", "Gain attenuation: {}", gainAttenuation);
    ...
    {
        ANT_CORE_ASSERT(m_Sources.find(sourceID) == m_Sources.end());
    ...
    bool Spatializer::ReleaseSource(uint32_t sourceID)
    {
        ...
        if (sIt == m_Sources.end())
        {
            ...
            //ANT_CORE_ERROR_TAG("Sound Spatializer", "Invalid sourceID {}.", sourceID);
            return false;
        }
    ...
    void Spatializer::UpdateSourcePosition(uint32_t sourceID, const Audio::Transform& transform, glm::vec3 velocity)
    {
        ...
        if (sIt == m_Sources.end())
        {
            ...
            //? Disabled the error massage to prevent the spam. Neen to not update position on audio objects with disabled spatializer.
            //ANT_CORE_ERROR_TAG("Sound Spatializer", "UpdateSourcePosition() Invalid sourceID {}.", sourceID);
            return;
        }
        ...
        //ANT_CORE_INFO_TAG("Sound Spatializer", "Relative Pos: X {0} | Y {1} | Z {2}", relativePos.x, relativePos.y, relativePos.z);

        //ANT_CORE_TRACE_TAG("Sound Spatializer", "Angle degrees: {}", glm::degrees(azimuth));
