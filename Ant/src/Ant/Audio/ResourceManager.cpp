...
namespace Ant::Audio {
	...
	void ResourceManager::Initialize()
	{
		...
		if (std::filesystem::exists(bankPath) && std::filesystem::is_regular_file(bankPath))
		{
			...
			ANT_CORE_ASSERT(m_SoundBank->IsLoaded());
		}
	}
	...
	void ResourceManager::ReleaseResources()
	{
		...
		if (!bankFilepath.empty())	ANT_CONSOLE_LOG_INFO("Sound Bank '{0}' has been unloaded.", bankFilepath);
		else if (anyUnloaded)		ANT_CONSOLE_LOG_INFO("Sounds loaded with Sound Bank has been unloaded.");
		else						ANT_CONSOLE_LOG_INFO("No sounds were unloaded with Sound Bank because none has been loaded.");
	}

	std::vector<AssetHandle> ResourceManager::CollectWaveHandlesForPlayAction(const TriggerAction& playAction) const
	{
		ANT_CORE_ASSERT(playAction.Type == EActionType::Play);
	...
	Ref<SoundBank> ResourceManager::BuildSoundBank()
	{
		...
		if (m_SoundBank)
		{
			...
			ANT_CONSOLE_LOG_INFO("Successfully packaged Sound Bank. Number of audio files packaged {0}, Sound Bank size {1}, file path {2}",
	...
	{
		ANT_CORE_ASSERT(!std::filesystem::exists(sourceFile));
