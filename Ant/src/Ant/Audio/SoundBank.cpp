...
namespace Ant {
		...
		: m_Path(path)
	{
		...
		if (memcmp(m_File.Header.HEADER, "ANTSB", 5) != 0)
			return;
		...
		ANT_CONSOLE_LOG_TRACE("Audio: SoundBank: Successfully loaded SoundBank. Number of files in the SoundBank - {}", m_File.Header.AudioFileCount);
	}
	...
	{
		ANT_CORE_VERIFY(!Application::IsRuntime());
		...
		// Error checking
		{
			...
			if (!errorMsg.empty())
			{
				ANT_CONSOLE_LOG_ERROR("Failed to create Sound Bank. {}", errorMsg);
				return nullptr;
			}
		}
		...
		for (const auto& handle : waveAssets)
		{
			...
			// Create file info entry
			{
				...
				ANT_CORE_WARN("SoundBank - {}", serializedPathString);
