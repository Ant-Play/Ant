...
#define NODE(name) { Identifier(Ant::Utils::CreateUserFriendlyTypeName(#name)), [](UUID nodeID) { return new name(#name, nodeID); } }
	...
	NodeProcessor* Factory::Create(Identifier nodeTypeID, UUID nodeID)
	{
		if (!NodeProcessors.count(nodeTypeID))
		{
			...
			if (!nodeTypeID.GetDBGName().empty())
				ANT_CORE_ERROR("SoundGraph::Factory::Create - Node with type ID {} is not in the registry", nodeTypeID.GetDBGName());
			else
				ANT_CORE_ERROR("SoundGraph::Factory::Create - Node with type ID {} is not in the registry", (uint32_t)nodeTypeID);
