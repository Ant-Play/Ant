...
#define LOG_PLAYBACK(...) ANT_CORE_INFO(__VA_ARGS__)
#else
#define LOG_PLAYBACK(...)
#endif
    ...
    bool Ant::SoundGraphSound::InitializeAudioCallback()
    {
        bool result = true;
    ...
    {
        ANT_CORE_ASSERT(!IsPlaying());
        ANT_CORE_ASSERT(!bIsReadyToPlay);
        ...
        {
            ANT_CORE_ASSERT(false, "Failed to initialize Audio Callback for SoundGraphSound.");
            return false;
        }
    ...
    bool SoundGraphSound::Play()
    {
        ...
        bFinished = false;
        ANT_CORE_ASSERT(!m_Source->IsFinished());
    ...
    {
        ANT_CORE_ASSERT(milliseconds > 0);
    ...
    void SoundGraphSound::Update(Ant::Timestep ts)
    {
        ANT_PROFILE_FUNC();
        ...
        switch (m_PlayState)
        {
	        ...
	        case EPlayState::Starting:
	            if (graph)
	            {
	                ...
	                {
	                    ANT_CORE_ASSERT("Failed to post Play event to SoundGraph instance.");
	                }
	            }
	#if 0
	            if (isNodePlaying())
	        ...
	        case EPlayState::Playing:
	        {
	            ...
	            if (time >= step)
	            {
	                ...
	                if (!m_Source->SetParameter("VolumePar", choc::value::Value(volume)))
	                    ANT_CORE_ERROR("Failed to set VolumePar");
	            }*/
    ...
    int SoundGraphSound::StopNow(bool notifyPlaybackComplete /*= true*/, bool resetPlaybackPosition /*= true*/)
    {
        ...
        if (resetPlaybackPosition)
        {
            ...
            ANT_CORE_ASSERT(m_PlayState != EPlayState::Starting);
            m_NextPlayState = EPlayState::Stopped;
        }
