...
namespace Ant
{
    bool RefillWavePlayerBuffer(Audio::WaveSource& waveSource, void* userData, uint32_t numFrames)
    {
        ...
        ANT_CORE_ASSERT(samplesToRefill <= 960 / 2);
    ...
    SoundGraphSource::SoundGraphSource()
    {
        ...
        m_HandleConsoleMessage = [&](uint64_t frameIndex, const char* message)
        {
            if (message[0] == '!')
            {
                ANT_CORE_ERROR(R"sgMessage([Sound Graph] Frame index: {0}
    Message: {1}", message, frameIndex)sgMessage", frameIndex, message);
            }
            else if (message[0] == '*')
            {
                ANT_CORE_WARN(R"sgMessage([Sound Graph] Frame index: {0}
    Message: {1}", message, frameIndex)sgMessage", frameIndex, message);
            }
            else
            {
                ANT_CORE_TRACE(R"sgMessage([Sound Graph] Frame index: {0}
    Message: {1}", message, frameIndex)sgMessage", frameIndex, message);
            }
        };
        ...
        m_HandleOutgoingEvent = [&](uint64_t frameIndex, Identifier endpointID, const choc::value::ValueView& eventData)
        {
            ...
            //? DBG
            //ANT_CORE_INFO("[SoundGraph Patch Event] Endpoint name: {0} | frame index: {1} | event data: {2}", endpointName, frameIndex, choc::json::toString(eventData));
    ...
    bool SoundGraphSource::Init(uint32_t sampleRate, uint32_t maxBlockSize, const BusConfig& config)
    {
        ...
        ANT_CORE_ASSERT(result == MA_SUCCESS);
        ...
        if (m_Graph != nullptr && m_Graph->IsPlayable() && !IsSuspended())
        {
            ...
            if (m_PresetIsInitialized)
            {
                ...
                if (flag_PlayRequested.CheckAndResetIfDirty())
                {
                    ...
                    //ANT_CORE_WARN("Play request sent");
                ...
                //ANT_CORE_ASSERT(numFramesRequested <= outBuffer[0].getNumFrames());
    ...
    bool SoundGraphSource::SetParameter(uint32_t parameterID, choc::value::ValueView value)
    {
        ...
        {
            //ANT_CONSOLE_LOG_INFO("Parameter: id {0}, value {1}", parameterInfo->first, choc::json::toString(value));
        ...
        {
            ANT_CONSOLE_LOG_WARN("SetParameter(parameterID {0}, value {1}) - Parameter with ID {0} doesn't exist in current context."
    ...
    bool SoundGraphSource::ApplyParameterPreset(const Utils::PropertySet& preset)
    {
        ...
        ANT_CORE_ASSERT(!m_ParameterHandles.empty());
        ...
        for (const auto& name : names)
        {
            ...
            {
                ANT_CORE_WARN_TAG("SoundGraphSource", "ApplyParameterPreset - SoundGraphSource doesn't have parameter '{}'", name);
                return false;
            }
        }
    ...
    void SoundGraphSource::UpdateParameterSet()
    {
        ...
        ANT_CORE_ASSERT(numParametersSet == totalNumberOfParameters);
    }

    bool SoundGraphSource::ApplyParameterPresetInternal()
    {
        ANT_CORE_ASSERT(m_Graph);
        ...
        for (const SoundGraphPatchPreset::Parameter& parameter : *presetSafe)
        {
            ...
            if (m_Graph->SendInputValue(parameter.Handle, parameter.Value, false))
            {
               ...
               /* ANT_CORE_INFO("parameter set: {}", param->Name);
                if (value.isFloat())
                    ANT_CORE_INFO("value: {}", value.get<float>());*/
            }
            else
            {
                ANT_CORE_ASSERT(false, "Failed to send input event to SoundGraph instance. Most likely endpoint handle was invalid.")
            }
        }
