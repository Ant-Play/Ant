...
#include "Ant/Audio/SoundGraph/WaveSource.h"
...
#if LOG_DBG_MESSAGES
#define DBG(...) ANT_CORE_WARN(__VA_ARGS__)
#else
#define DBG(...)
#endif
...
namespace Ant::SoundGraph {
	...
	private:
		...
		void StartPlayback()
		{
			if (!isInitialized)
			{
				ANT_CORE_WARN("!WavePlayer::Play() - Wave Player is not initialized.");
				return;
			}

			startSample = int64_t(*in_StartTime * sampleRate /*currentBuffer.sampleRate*/);
			ANT_CORE_ASSERT(startSample >= 0);
			...
			if (!buffer.WaveHandle)
			{
				...
				//ANT_CORE_WARN("!WavePlayer::Play() - invalid wave source id.");
	...
	private:
		...
		{
			memset(&out_OutLeft, 0, sizeof(&out_OutLeft));
		}
	};
}
