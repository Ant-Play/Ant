...
namespace Ant {
	...
	namespace Utils
	{
		...
		static inline void CopyArrayData(const choc::value::ValueView& destCustomArrayObject, const choc::value::ValueView& sourceArrayData)
		{
			...
			ANT_CORE_ASSERT(destination.size() >= sourceArrayData.size());
			ANT_CORE_ASSERT(currentDataSize >= newDataSize);
	...
	SoundGraphPatchPreset::Parameter* Ant::SoundGraphPatchPreset::begin() { return m_Parameters.begin(); }
	SoundGraphPatchPreset::Parameter* Ant::SoundGraphPatchPreset::end() { return m_Parameters.end(); }
	...
	void SoundGraphPatchPreset::TranslateParameters()
	{
		...
		for (auto& parameter : m_Parameters)
		{
			...
			else if (value.isArray())
			{
				...
				//ANT_CORE_ASSERT(arrayMaxSize == value.size());
	...
	bool SoundGraphPatchPreset::SetParameter(std::string_view name, const choc::value::ValueView& value)
	{
		...
		{
			ANT_CORE_ASSERT(false, "Dynamically changing SoundGraph Wave asset arrays is not implemented yet!");
			return false;
		}
