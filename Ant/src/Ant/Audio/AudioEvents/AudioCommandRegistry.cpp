...
namespace Ant {
	...
	void AudioCommandRegistry::Init(WeakRef<AudioCommandRegistry>& instance)
	{
		if (!instance)
		{
			ANT_CORE_ASSERT(false, "Invalid instance of AudioCommandsRegistry.");
			return;
		}
	...
	bool AudioCommandRegistry::AddNewCommand(Audio::ECommandType type, const char* uniqueName)
	{
		if (!s_ActiveRegistry)
		{
	...
	void AudioCommandRegistry::LoadCommandRegistry(const std::filesystem::path& filepath)
	{
		...
		*/
		...
		if (!triggers)
		{
			ANT_CORE_ERROR("CommandRegistry appears to be corrupted!");
			return;
		}
		...
		for (auto entry : triggers)
		{
			...
			ANT_DESERIALIZE_PROPERTY(DebugName, trigger.DebugName, entry, std::string(""));

			if (trigger.DebugName.empty())
			{
				ANT_CORE_ERROR("Tried to load invalid command. Command must have a DebugName, or an ID!");
				continue;
			}
			...
			for (auto actionEntry : actions)
			{
				...
				ANT_DESERIALIZE_PROPERTY(Type, type, actionEntry, std::string(""));
				ANT_DESERIALIZE_PROPERTY_ASSET(Target, action.Target, actionEntry, SoundConfig);
				ANT_DESERIALIZE_PROPERTY(Context, context, actionEntry, std::string(""));
	...
	void AudioCommandRegistry::WriteRegistryToFile(const std::filesystem::path& filepath)
	{
		...
		for (auto& [commandID, trigger] : sortedTriggers)
		{
			...
			ANT_SERIALIZE_PROPERTY(DebugName, trigger.DebugName, out);

			out << YAML::Key << "Actions" << YAML::BeginSeq;
			for (auto& action : trigger.Actions.GetVector())
			{
				out << YAML::BeginMap;
				ANT_SERIALIZE_PROPERTY(Type, Utils::AudioActionTypeToString(action.Type), out);
				ANT_SERIALIZE_PROPERTY_ASSET(Target, action.Target, out);
				ANT_SERIALIZE_PROPERTY(Context, Utils::AudioActionContextToString(action.Context), out);
				out << YAML::EndMap;
			}
