...
namespace Ant {
	...
	*/
	...
	struct EntityIDMap
	{
		...
		{
			std::scoped_lock lock{ m_Mutex };
			ANT_CORE_ASSERT(m_EntityIDMap.at(sceneID).find(entityID) != m_EntityIDMap.at(sceneID).end(),
...
namespace std {
	template <>
	struct hash<Ant::Audio::EventID>
	{
		std::size_t operator()(const Ant::Audio::EventID& id) const noexcept
		{
...
//namespace std {
//	// Implementation
//	 std::size_t hash<Ant::Audio::EventID>::operator()(const Ant::Audio::EventID& id) const noexcept { return hash<uint32_t>()(id.m_ID); }
//}