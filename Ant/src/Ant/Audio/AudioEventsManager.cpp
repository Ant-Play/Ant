...
namespace Ant::Audio {
	...
	bool AudioEventsManager::ProcessTriggerCommand(EventInfo& info)
	{
		...
		auto processPlayAction = [&](TriggerAction& action)
		{
			...
			{
				ANT_CORE_ERROR_TAG("Audio", "Failed to initialize new Voice for audio object");
		...
		auto processPauseAction = [&](TriggerAction& action)
		{
			if (action.Context == EActionContext::GameObject)
			{
				...
				{
					ANT_CORE_ERROR_TAG("Audio", "No Active Sources for object to Pause");
				}
			}
			else // Global context
			{
		...
		auto processStopAction = [&](TriggerAction& action)
		{
			if (action.Context == EActionContext::GameObject)
			{
				...
				{
					ANT_CORE_ERROR_TAG("Audio", "No Active Sources for object to Stop");
				}
			}
			else // Global context
			{
		...
		for (auto& action : trigger.Actions.GetVector())
		{
			...
			{
				ANT_CORE_ERROR_TAG("Audio", "One of the actions of audio trigger ({0}) doesn't have a target assigned!", trigger.DebugName);
				action.Handled = true;
				continue;
			}
	...
	bool AudioEventsManager::ExecuteActionOnPlayingEvent(EventID playingEvent, EActionTypeOnPlayingEvent action)
	{
		...
		{
			ANT_CONSOLE_LOG_WARN("Audio. Trying to execute action {} on playing event that's not in the active events registry.", magic_enum::enum_name(action));
			return false;
		}
