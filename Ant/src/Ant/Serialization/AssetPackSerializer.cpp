...
namespace Ant{
	...
	{
		// Print Info
		ANT_CORE_TRACE("Serializing AssetPack to {}", path.string());
		ANT_CORE_TRACE("  {} scenes", file.Index.Scenes.size());
		uint32_t assetCount = 0;
		for (const auto& [sceneHandle, sceneInfo] : file.Index.Scenes)
			assetCount += sceneInfo.Assets.size();
		ANT_CORE_TRACE("  {} assets (including duplicates)", assetCount);
		...
		ANT_CORE_TRACE("Serialized {} assets into AssetPack", serializedAssets.size());
	...
	{
		// Print Info
		ANT_CORE_TRACE("Deserializing AssetPack from {}", path.string());
		...
		bool validHeader = memcmp(file.Header.HEADER, "ANTAP", 5) == 0;
		ANT_CORE_ASSERT(validHeader);
		if (!validHeader)
			return false;
		...
		ANT_CORE_TRACE("Deserialized index with {} scenes from AssetPack", sceneCount);
		return true;
	}
