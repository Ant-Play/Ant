...
namespace Ant{
	...
	void AssetPack::AddAsset(Ref<Asset> asset)
	{
		...
		{
			ANT_CORE_WARN_TAG("AssetPack", "Asset already present in asset pack");
			return;
		}
	...
	Buffer AssetPack::ReadAppBinary()
	{
		...
		ANT_CORE_VERIFY(m_File.Index.PackedAppBinarySize == (buffer.Size + sizeof(uint32_t)));
		return buffer;
	}
...
#define DEBUG_PRINT 1
		...
		for (const auto& [handle, metadata] : registry)
		{
			if (metadata.Type == AssetType::Scene)
			{
				...
				ANT_CORE_TRACE("Deserializing Scene: {}", metadata.FilePath);
				if (serializer.Deserialize(Project::GetAssetDirectory() / metadata.FilePath))
				{
					...
					ANT_CORE_TRACE("  Scene has {} used assets", sceneAssetList.size());
				...
				{
					ANT_CONSOLE_LOG_ERROR("Failed to deserialize Scene: {} ({})", metadata.FilePath, handle);
				}
		...
		ANT_CONSOLE_LOG_INFO("Project contains {} used assets", fullAssetList.size());

#if DEBUG_PRINT
		ANT_CORE_TRACE("Complete AssetPack:");

		for (AssetHandle handle : fullAssetList)
		{
			const auto& metadata = Project::GetEditorAssetManager()->GetMetadata(handle);
			ANT_CORE_TRACE("{}: {} ({})", Utils::AssetTypeToString(metadata.Type), metadata.FilePath, metadata.Handle);
		}
#endif

		Buffer appBinary = FileSystem::ReadBytes(Project::GetScriptModuleFilePath());

		AssetPackSerializer::Serialize(Project::GetAssetDirectory() / "AssetPack.aap", assetPackFile, appBinary, progress);
		progress = 1.0f;
		return nullptr;
	}
	...
	{
		return Load(Project::GetAssetDirectory() / "AssetPack.aap");
	}
	...
	Ref<AssetPack> AssetPack::Load(const std::filesystem::path& path)
	{
		...
		{
			ANT_CORE_INFO("-----------------------------------------------------");
			ANT_CORE_INFO("AssetPack Dump {}", assetPack->m_Path);
			ANT_CORE_INFO("-----------------------------------------------------");
			std::unordered_map<AssetType, uint32_t> typeCounts;
			std::unordered_set<AssetHandle> duplicatePreventionSet;
			for (const auto& [sceneHandle, sceneInfo] : index.Scenes)
			{
				ANT_CORE_INFO("Scene {}:", sceneHandle);
				for (const auto& [assetHandle, assetInfo] : sceneInfo.Assets)
				{
					ANT_CORE_INFO("  {} - {}", Utils::AssetTypeToString((AssetType)assetInfo.Type), assetHandle);
			...
			ANT_CORE_INFO("-----------------------------------------------------");
			ANT_CORE_INFO("Summary:");
			for (const auto& [type, count] : typeCounts)
			{
				ANT_CORE_INFO("  {} {}", count, Utils::AssetTypeToString(type));
			}
			ANT_CORE_INFO("-----------------------------------------------------");
		}
#endif
