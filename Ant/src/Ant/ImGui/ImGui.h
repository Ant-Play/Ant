...
#include "Ant/Editor/AssetEditorPanelInterface.h"
...
#include "Ant/Scripts/ScriptCache.h"

#include "choc/text/choc_StringUtilities.h"

namespace Ant::UI {

#define ANT_MESSAGE_BOX_OK_BUTTON BIT(0)
#define ANT_MESSAGE_BOX_CANCEL_BUTTON BIT(1)
#define ANT_MESSAGE_BOX_USER_FUNC BIT(2)
	...
	static void ShowMessageBox(const char* title, const std::function<void()>& renderFunction, uint32_t width = 600, uint32_t height = 0)
	{
		...
		messageBoxData.Flags = ANT_MESSAGE_BOX_USER_FUNC;
	...
	static void RenderMessageBoxes()
	{
		for (auto& [key, messageBoxData] : s_MessageBoxes)
		{
			...
			{
				if (messageBoxData.Flags & ANT_MESSAGE_BOX_USER_FUNC)
				{
					ANT_CORE_VERIFY(messageBoxData.UserRenderFunction, "No render function provided for message box!");
				...
				else
				{
					...
					if (messageBoxData.Flags & ANT_MESSAGE_BOX_OK_BUTTON)
					{
						if (ImGui::Button("Ok"))
							ImGui::CloseCurrentPopup();

						if (messageBoxData.Flags & ANT_MESSAGE_BOX_CANCEL_BUTTON)
							ImGui::SameLine();
					}

					if (messageBoxData.Flags & ANT_MESSAGE_BOX_CANCEL_BUTTON && ImGui::Button("Cancel"))
					{
						ImGui::CloseCurrentPopup();
					}
				}
	...
	static bool DrawFieldValue(Ref<Scene> sceneContext, const std::string& fieldName, Ref<FieldStorage>& storage)
	{
		...
		/*if (field->HasAttribute("Ant.ClampValueAttribute"))
		{
			MonoObject* attr = field->GetAttribute("Ant.ClampValueAttribute");
			ANT_TRY_GET_FIELD_VALUE(min, "Ant.ClampValueAttribute", "Min", attr);
			ANT_TRY_GET_FIELD_VALUE(max, "Ant.ClampValueAttribute", "Max", attr);
		}*/
