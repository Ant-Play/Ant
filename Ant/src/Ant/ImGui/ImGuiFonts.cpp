...
namespace Ant::UI {
	...
	void Fonts::Add(const FontConfiguration& config, bool isDefault)
	{
		...
		{
			ANT_CORE_WARN_TAG("EditorUI", "Tried to add font with name '{0}' but that name is already taken!", config.FontName);
			return;
		}
		...
		ANT_CORE_VERIFY(font, "Failed to load font file!");
	...
	{
		ANT_CORE_VERIFY(s_Fonts.find(fontName) != s_Fonts.end(), "Failed to find font with that name!");
