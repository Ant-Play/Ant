...
#include "Ant/Scripts/ScriptProfiler.h"
...
namespace Ant {
	...
	void ScriptEngineDebugPanel::OnProjectChanged(const Ref<Project>& project)
	{
		...
		const auto& antCoreAssembly = ScriptEngine::GetCoreAssemblyInfo();
		const auto& appAssembly = ScriptEngine::GetAppAssemblyInfo();
		s_LoadedAssembliesMetadata.push_back(antCoreAssembly->Metadata);
		s_LoadedAssembliesMetadata.push_back(appAssembly->Metadata);

		for (const auto& referencedAssemblyMetadata : antCoreAssembly->ReferencedAssemblies)
