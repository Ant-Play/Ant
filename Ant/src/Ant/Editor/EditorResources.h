...
namespace Ant {
	...
	public:
		...
		// Textures
		inline static Ref<Texture2D> AntLogoTexture = nullptr;
		...
		static void Init()
		{
			...
			// Textures
			AntLogoTexture = LoadTexture("H_logo.png");
	...
	private:
		...
		static Ref<Texture2D> LoadTexture(const std::filesystem::path& relativePath, const std::string& name, TextureSpecification specification)
		{
			...
			{
				ANT_CORE_FATAL("Failed to load icon {0}! The file doesn't exist.", path.string());
