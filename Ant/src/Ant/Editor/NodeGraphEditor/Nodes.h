...
#include "Ant/Reflection/TypeName.h"
...
namespace Ant {
	...
	// TODO: this utility could be moved out of this header
	//=================================================================
	...
	*/
	...
	template<> static choc::value::Value Ant::ValueFrom(const TFlow& obj) { return choc::value::createObject(type::type_name<TFlow>()); }
	template<> static choc::value::Value Ant::ValueFrom(const TFunction& obj) { return choc::value::createObject(type::type_name<TFunction>()); }
	template<> static choc::value::Value Ant::ValueFrom(const TDelegate& obj) { return choc::value::createObject(type::type_name<TDelegate>()); }

	template<> static choc::value::Value Ant::ValueFrom(const UUID& obj)
	{
	...
	*/
	...
	//=================================================================
	/**
		Pin base. Using 'int typeID' for type-erased imlementation
		specific enum types.
	*/
	struct Pin
	{
	...
	*/
	template<int EType, class TValueType, ImU32 Colour = IM_COL32_WHITE, class ETypeEnum = EPinType>
	struct PinDescr : Pin
	{
		...
		//virtual ImU32 GetColour() const override { return colour; }
	};
	...
	*/
	...
	struct Node
	{
		...
		std::vector<Pin*> Inputs;		// TODO: these Ins and Outs can be assigned by the subclass
		std::vector<Pin*> Outputs;
		ImColor Color;
		NodeType Type;					// TODO: this could also be implmentation specific types
		ImVec2 Size;
...
#define DECLARE_GRAPH_NODE(NodeType, Inputs, Outputs)											\
	...
	namespace Nodes {
		...
		public:
			...
			static const std::unordered_map<std::string, const std::vector<Token>*> EnumTokensRegistry;
			...
			[[nodiscard]] static Node* SpawnNodeStatic(std::string_view category, std::string_view type)
			{
				...
				if (cat != Registry.end())
				{
					...
					{
						ANT_CORE_ERROR("SpawnNodeStatic() - Category {0} does not contain node type {1}", category, type);
						return nullptr;
					}
				}

				ANT_CORE_ERROR("SpawnNodeStatic() - Category {0} does not exist", category);
				return nullptr;
			...
			static const std::vector<Token>* GetEnumTokens(std::string_view nodeName, std::string_view memberName)
			{
				...
				ANT_CORE_ASSERT(memberName.find("::") == std::string_view::npos);

				// TODO: for now this is specific to SoundGraph nodes
				const std::string fullName = choc::text::joinStrings<std::array<std::string_view, 2>>({ Utils::CreateUserFriendlyTypeName(nodeName), Impl::RemovePrefixAndSuffix(memberName) }, "::");
