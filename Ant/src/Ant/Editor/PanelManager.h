...
namespace Ant {
	...
	// NOTE: For now this just corresponds to what menu the panel will be listed in (in AntPlay)
	...
	public:
		...
		Ref<TPanel> AddPanel(PanelCategory category, const PanelData& panelData)
		{
			...
			{
				ANT_CORE_ERROR_TAG("PanelManager", "A panel with the id '{0}' has already been added.", panelData.ID);
				return nullptr;
			}
		...
		Ref<TPanel> GetPanel(const char* strID)
		{
			...
			ANT_CORE_ERROR_TAG("PanelManager", "Couldn't find panel with id '{0}'", strID);
			return nullptr;
		}
