...
namespace Ant {
	...
	{
		ANT_CORE_ASSERT(s_Instance == nullptr);
		s_Instance = this;
	...
	void EditorConsolePanel::RenderMenu(const ImVec2& size)
	{
		...
		const std::string clearOnPlayText = fmt::format("{} Clear on Play", m_ClearOnPlay ? ANT_ICON_CHECK : ANT_ICON_TIMES);
...

		{
			...
			if (UI::ColoredButton(ANT_ICON_INFO_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Info), textColor, buttonSize))
			...
			if (UI::ColoredButton(ANT_ICON_EXCLAMATION_TRIANGLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Warning), textColor, buttonSize))
			...
			if (UI::ColoredButton(ANT_ICON_EXCLAMATION_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Error), textColor, buttonSize))
