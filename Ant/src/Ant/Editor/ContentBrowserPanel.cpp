...
namespace Ant{
	...
	ContentBrowserPanel::ContentBrowserPanel()
	{
		...
		m_AssetIconMap["asc"] = AssetManager::GetAsset<Texture2D>("assets/editor/hazel.png");
				...
				ImGui::BeginChild("Scrolling");
				{
					...
					if (ImGui::BeginPopupContextWindow(0, 1 | ImGuiPopupFlags_NoOpenOverItems))
					{
						if (ImGui::BeginMenu("Create"))
						{
							...
							{
								ANT_CORE_INFO("Creating Scene...");
							}

							if (ImGui::MenuItem("Script"))
							{
								ANT_CORE_INFO("Creating Script...");
							}

							if (ImGui::MenuItem("Prefab"))
							{
								ANT_CORE_INFO("Creating Prefab...");
							}

							if (ImGui::MenuItem("Physics Material"))
							{
								AssetManager::CreateNewAsset<PhysicsMaterial>("New Physics Material.apm", AssetType::PhysicsMat, m_CurrentDirHandle, 0.6f, 0.6f, 0.0f);
								UpdateCurrentDirectory(m_CurrentDirHandle);
							}
	...
	void ContentBrowserPanel::HandleRenaming(Ref<Asset>& asset)
	{
		...
		if (m_RenamingSelected)
		{
			...
			{
				ANT_CORE_INFO("Renaming to {0}", m_RenameBuffer);
