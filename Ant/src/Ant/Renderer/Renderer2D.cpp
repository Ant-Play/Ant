...
namespace Ant{
	...
	void Renderer2D::Init()
	{
		...
		
		{
			...
			{
				m_QuadVertexBuffer[i] = VertexBuffer::Create(MaxVertices * sizeof(QuadVertex));
				m_QuadVertexBufferBase[i] = anew QuadVertex[MaxVertices];
			}
			

			uint32_t* quadIndices = anew uint32_t[MaxIndices];
		...
		// Lines
		{
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				m_LineVertexBuffer[i] = VertexBuffer::Create(MaxLineVertices * sizeof(LineVertex));
				m_LineVertexBufferBase[i] = anew LineVertex[MaxLineVertices];
			}

			uint32_t* lineIndices = anew uint32_t[MaxLineIndices];
		...
		// Text
		{
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				m_TextVertexBuffer[i] = VertexBuffer::Create(MaxVertices * sizeof(TextVertex));
				m_TextVertexBufferBase[i] = anew TextVertex[MaxVertices];
			}

			uint32_t* textQuadIndices = anew uint32_t[MaxIndices];
		...
		// Circles
		{
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				m_CircleVertexBuffer[i] = VertexBuffer::Create(MaxVertices * sizeof(QuadVertex));
				m_CircleVertexBufferBase[i] = anew CircleVertex[MaxVertices];
			}
		}
	...
	void Renderer2D::BeginScene(const glm::mat4& viewProj, const glm::mat4& view, bool depthTest)
	{
		...
		ANT_CORE_TRACE_TAG("Renderer", "Renderer2D::BeginScene frame {}", frameIndex);
	...
	void Renderer2D::EndScene()
	{
		...
		ANT_CORE_TRACE_TAG("Renderer", "Renderer2D::EndScene frame {}", frameIndex);
		ANT_CORE_ASSERT(m_QuadVertexBufferPtr >= m_QuadVertexBufferBase[frameIndex]);
		uint32_t dataSize = (uint32_t)((uint8_t*)m_QuadVertexBufferPtr - (uint8_t*)m_QuadVertexBufferBase[frameIndex]);
		ANT_CORE_TRACE_TAG("Renderer", "Data size: {}", dataSize);
		if (dataSize)
		{
...
#pragma warning(default : 4996)
	...
	void Renderer2D::DrawString(const std::string& string, const Ref<Font>& font, const glm::mat4& transform, float maxWidth, const glm::vec4& color, float lineHeightOffset, float kerningOffset)
	{
		...
		// TODO: these font metrics really should be cleaned up/refactored...
