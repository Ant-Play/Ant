...
#include "Ant/Core/RenderThread.h"
...
namespace Ant{
	...
	public:
		...
		static void LightCulling(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<PipelineCompute> computePipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material, const glm::uvec3& workGroups);
		static void DispatchComputeShader(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<PipelineCompute> computePipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material, const glm::uvec3& workGroups, Buffer additionalUniforms = {});
	...
	namespace Utils {
		...
		inline void DumpGPUInfo()
		{
			...
			ANT_CORE_TRACE_TAG("Renderer", "GPU Info:");
			ANT_CORE_TRACE_TAG("Renderer", "  Vendor: {0}", caps.Vendor);
			ANT_CORE_TRACE_TAG("Renderer", "  Device: {0}", caps.Device);
			ANT_CORE_TRACE_TAG("Renderer", "  Version: {0}", caps.Version);
		}
	}
