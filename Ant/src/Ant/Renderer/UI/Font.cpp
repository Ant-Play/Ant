...
#define THREADS 8
	...
	static void CacheFontAtlas(const std::string& fontName, float fontSize, AtlasHeader header, const void* pixels)
	{
		...
		{
			stream.close();
			ANT_CORE_ERROR_TAG("Renderer", "Failed to cache font atlas to {0}", filepath.string());
			return;
		}
	...
	void Font::CreateAtlas(Buffer buffer)
	{
		...
		ANT_CORE_ASSERT(glyphsLoaded >= 0);
		ANT_CORE_TRACE_TAG("Renderer", "Loaded geometry of {0} out of {1} glyphs", glyphsLoaded, (int)charset.size());
		// List missing glyphs
		if (glyphsLoaded < (int)charset.size())
		{
			ANT_CORE_WARN_TAG("Renderer", "Missing {0} {1}", (int)charset.size() - glyphsLoaded, fontInput.glyphIdentifierType == GlyphIdentifierType::UNICODE_CODEPOINT ? "codepoints" : "glyphs");
		}
		...
		if (int remaining = atlasPacker.pack(m_MSDFData->Glyphs.data(), (int)m_MSDFData->Glyphs.size()))
		{
			...
			{
				ANT_CORE_ERROR_TAG("Renderer", "Error: Could not fit {0} out of {1} glyphs into the atlas.", remaining, (int)m_MSDFData->Glyphs.size());
				ANT_CORE_ASSERT(false);
			}
		}
		atlasPacker.getDimensions(config.width, config.height);
		ANT_CORE_ASSERT(config.width > 0 && config.height > 0);
		config.emSize = atlasPacker.getScale();
		config.pxRange = atlasPacker.getPixelRange();
		if (!fixedScale)
			ANT_CORE_TRACE_TAG("Renderer", "Glyph size: {0} pixels/EM", config.emSize);
		if (!fixedDimensions)
			ANT_CORE_TRACE_TAG("Renderer", "Atlas dimensions: {0} x {1}", config.width, config.height);
