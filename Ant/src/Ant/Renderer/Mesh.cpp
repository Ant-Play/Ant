...
#if MESH_DEBUG_LOG
#define ANT_MESH_LOG(...) ANT_CORE_TRACE(__VA_ARGS__)
#else
#define ANT_MESH_LOG(...)
#endif
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			ANT_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						ANT_MESH_LOG("Found existing bone in map");
	...
	{
		ANT_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		ANT_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
