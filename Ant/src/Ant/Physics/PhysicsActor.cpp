...
namespace Ant{
	...
	float PhysicsActor::GetMass() const
	{
		if (!IsDynamic())
		{
			ANT_CORE_WARN("Trying to set mass of non-dynamic PhysicsActor.");
			return 0.0F;
		}
	...
	void PhysicsActor::AddForce(const glm::vec3& force, ForceMode forceMode)
	{
		if (!IsDynamic())
		{
			ANT_CORE_WARN("Trying to add force to non-dynamic PhysicsActor.");
			return;
		}
	...
	void PhysicsActor::AddTorque(const glm::vec3& torque, ForceMode forceMode)
	{
		if (!IsDynamic())
		{
			ANT_CORE_WARN("Trying to add torque to non-dynamic PhysicsActor.");
			return;
		}
