...
namespace Ant{
	...
	void PhysicsErrorCallback::reportError(physx::PxErrorCode::Enum code, const char* message, const char* file, int line)
	{
		...
		switch (code)
		{
			...
			case physx::PxErrorCode::eDEBUG_INFO:
				ANT_CORE_INFO("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
			case physx::PxErrorCode::eDEBUG_WARNING:
			case physx::PxErrorCode::ePERF_WARNING:
				ANT_CORE_WARN("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
			case physx::PxErrorCode::eINVALID_PARAMETER:
			case physx::PxErrorCode::eINVALID_OPERATION:
			case physx::PxErrorCode::eOUT_OF_MEMORY:
			case physx::PxErrorCode::eINTERNAL_ERROR:
				ANT_CORE_ERROR("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
				break;
			case physx::PxErrorCode::eABORT:
			case physx::PxErrorCode::eMASK_ALL:
				ANT_CORE_FATAL("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
	...
	void ContactListener3D::onWake(physx::PxActor** actors, physx::PxU32 count)
	{
		for(uint32_t i = 0; i < count; i++)
		{
			...
			ANT_CORE_INFO("PhysX Actor waking up: ID: {0}, Name: {1}", entity.GetUUID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	void ContactListener3D::onSleep(physx::PxActor** actors, physx::PxU32 count)
	{
		for(uint32_t i = 0; i < count; i++)
		{
			physx::PxActor& actor = *actors[i];
			Entity& entity = *(Entity*)actor.userData;
			ANT_CORE_INFO("PhysX Actor going to sleep: ID: {0}, Name: {1}", entity.GetUUID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	static physx::PxBroadPhaseType::Enum AntToPhysXBroadphaseType(BroadphaseType type)
	{
		switch (type)
		{
	...
	static physx::PxFrictionType::Enum AntToPhysXFrictionType(FrictionType type)
	{
		switch (type)
		{
	...
	physx::PxScene* PXPhysicsWrappers::CreateScene()
	{
		...
		sceneDesc.broadPhaseType = AntToPhysXBroadphaseType(settings.BroadphaseAlgorithm);
		sceneDesc.cpuDispatcher = s_CPUDispatcher;
		sceneDesc.filterShader = AntFilterShader;
		sceneDesc.simulationEventCallback = &s_ContactListener;
		sceneDesc.frictionType = AntToPhysXFrictionType(settings.FrictionModel);
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateConvexMesh(MeshColliderComponent& collider, const glm::vec3& scale, bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collider.CollisionMesh->GetFilePath()))
		{
			...
			for (const auto& submesh : collider.CollisionMesh->GetSubmeshes())
			{
				...
				{
					ANT_CORE_ERROR("Failed to cook convex mesh {0}", submesh.MeshName);
					continue;
				}
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateTriangleMesh(MeshColliderComponent& collider, const glm::vec3& scale, bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collider.CollisionMesh->GetFilePath()))
		{
			...
			for (const auto& submesh : collider.CollisionMesh->GetSubmeshes())
			{
				...
				{
					ANT_CORE_ERROR("Failed to cook triangle mesh: {0}", submesh.MeshName);
					continue;
				}
	...
	{
		ANT_CORE_ASSERT(!s_Foundation, "PXPhysicsWrappers::Initializer shouldn't be called more than once!");

		s_Foundation = PxCreateFoundation(PX_PHYSICS_VERSION, s_Allocator, s_ErrorCallback);
		ANT_CORE_ASSERT(s_Foundation, "PxCreateFoundation Failed!");
		...
		ANT_CORE_ASSERT(s_Physics, "PxCreatePhysics Failed!");

		s_CookingFactory = PxCreateCooking(PX_PHYSICS_VERSION, *s_Foundation, s_Physics->getTolerancesScale());
		ANT_CORE_ASSERT(s_CookingFactory, "PxCreatePhysics Failed!");
	...
	{
		ANT_CORE_ERROR("[PhysX Error]: {0}:{1} - {2}", file, line, exp);
