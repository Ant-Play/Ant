...
namespace Ant{
	...
	{
		ANT_CORE_VERIFY(m_Specification.Width > 0 && m_Specification.Height > 0);
	}
			...
			Renderer::SubmitResourceFree([info, layerViews = m_PerLayerImageViews]()
				{
					...
					ANT_CORE_WARN_TAG("Renderer", "VulkanImage2D::Release ImageView = {0}", (const void*)info.ImageView);
				});
	...
	{
		ANT_CORE_ASSERT(m_Specification.Layers > 1);
	...
	void VulkanImage2D::RT_CreatePerSpecificLayerImageViews(const std::vector<uint32_t>& layerIndices)
	{
		...
		//ANT_CORE_ASSERT(m_PerLayerImageViews.size() == m_Specification.Layers);
	...
	void VulkanImage2D::UpdateDescriptor()
	{
		...
		//ANT_CORE_WARN_TAG("Renderer", "VulkanImage2D::UpdateDescriptor to ImageView = {0}", (const void*)m_Info.ImageView);
	}
