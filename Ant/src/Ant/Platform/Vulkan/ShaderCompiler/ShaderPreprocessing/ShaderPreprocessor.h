...
namespace std {
	...
	template<>
	struct hash<Ant::IncludeData>
	{
		std::size_t operator()(const Ant::IncludeData& data) const noexcept
		{
...
namespace Ant {
		...
		while (startOfShaderStage != std::string::npos)
		{
			...
			if (tokens[index] == "#")
			{
				...
				if (tokens[index] == "pragma")
				{
					...
					{
						ANT_CORE_VERIFY(tokens[++index] == ":", "Stage pragma is invalid");

						// Skipped ':'
						const std::string_view stage(tokens[++index]);
						ANT_CORE_VERIFY(stage == "vert" || stage == "frag" || stage == "comp", "Invalid shader type specified");
						...
						const bool alreadyIncluded = std::find_if(includeData.begin(), includeData.end(), [fullPath, foundStage](const IncludeData& data)
							...
							if (isGuarded && alreadyIncluded)
								...
								ANT_CORE_WARN("\"{}\" Header does not contain a header guard (#pragma once).", fullPath);
				...
				{
					++index;
					if (tokens[index].rfind("__ANT_", 0) == 0) // Ant special macros start with "__ANT_"
					{
				...
				else if (tokens[index] == "if" || tokens[index] == "define") // Consider "#if defined()" too?
				{
					...
					{
						if (tokens[i].rfind("__ANT_", 0) == 0) // Ant special macros start with "__ANT_"
						{
	...
	std::map<VkShaderStageFlagBits, std::string> ShaderPreprocessor::PreprocessShader(const std::string& source, std::unordered_set<std::string>& specialMacros)
	{
		...
		ANT_CORE_ASSERT(newSource.size(), "Shader is empty!");
		...
		if constexpr (Lang == ShaderUtils::SourceLang::GLSL)
		{
			...
			ANT_CORE_VERIFY(tokens.size() >= 3 && tokens[1] == "version", "Invalid #version encountered or #version is NOT encounted first.");
		...
		ANT_CORE_VERIFY(stagePositions.size(), "Could not pre-process shader! There are no known stages defined in file.");
