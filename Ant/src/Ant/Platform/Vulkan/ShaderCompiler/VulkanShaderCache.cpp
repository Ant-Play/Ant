...
#include "Ant/Platform/Vulkan/VulkanShaderUtils.h"
...
namespace Ant {
		...
		for (auto& [filepath, shader] : shaderCache)
		{
			...
			for (auto& [stage, stageData] : shader)
			{
				...
				for (auto& header : stageData.Headers)
				{
					...
					ANT_SERIALIZE_PROPERTY(HeaderPath, header.IncludedFilePath.string(), out);
					ANT_SERIALIZE_PROPERTY(IncludeDepth, header.IncludeDepth, out);
					ANT_SERIALIZE_PROPERTY(IsRelative, header.IsRelative, out);
					ANT_SERIALIZE_PROPERTY(IsGaurded, header.IsGuarded, out);
					ANT_SERIALIZE_PROPERTY(HashValue, header.HashValue, out);
	...
	void VulkanShaderCache::Deserialize(std::map<std::string, std::map<VkShaderStageFlagBits, StageData>>& shaderCache)
	{
		...
		if (handles.IsNull())
		{
			ANT_CORE_ERROR("[ShaderCache] Shader Registry is invalid.");
			return;
		}
		...
		if (handles.IsMap())
		{
			ANT_CORE_ERROR("[ShaderCache] Old Shader Registry format.");
			return;
		}
		...
		{
			std::string path;
			ANT_DESERIALIZE_PROPERTY(ShaderPath, path, shader, std::string());
			for (auto stage : shader["Stages"]) //Stages
			{
				std::string stageType;
				uint32_t stageHash;
				ANT_DESERIALIZE_PROPERTY(Stage, stageType, stage, std::string());
				ANT_DESERIALIZE_PROPERTY(StageHash, stageHash, stage, 0u);
				...
				for (auto& header : stage["Headers"])
				{
					...
					bool isGuarded;
					uint32_t hashValue;
					ANT_DESERIALIZE_PROPERTY(HeaderPath, headerPath, header, std::string());
					ANT_DESERIALIZE_PROPERTY(IncludeDepth, includeDepth, header, 0u);
					ANT_DESERIALIZE_PROPERTY(IsRelative, isRelative, header, false);
					ANT_DESERIALIZE_PROPERTY(IsGaurded, isGuarded, header, false);
					ANT_DESERIALIZE_PROPERTY(HashValue, hashValue, header, 0u);
