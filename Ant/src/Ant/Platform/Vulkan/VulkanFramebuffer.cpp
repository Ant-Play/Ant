...
namespace Ant{
		...
		: m_Specification(specification)
	{
		...
		ANT_CORE_ASSERT(specification.Attachments.Attachments.size());
	...
	{
		// ANT_CORE_TRACE("VulkanFramebuffer::RT_Invalidate ({})", m_Specification.DebugName);
		...
		for (auto attachmentSpec : m_Specification.Attachments.Attachments)
		{
			if (Utils::IsDepthFormat(attachmentSpec.Format))
			{
				...
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					...
					ANT_CORE_ASSERT(Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach non-depth image as depth attachment");
			...
			{
				//ANT_CORE_ASSERT(!m_Specification.ExistingImage, "Not supported for color attachments");
				...
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					...
					ANT_CORE_ASSERT(!Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach depth image as color attachment");
		...
		if (m_DepthAttachmentImage)
		{
			...
			if (m_Specification.ExistingImage)
			{
				ANT_CORE_ASSERT(m_Specification.ExistingImageLayers.size() == 1, "Depth attachments do not support deinterleaving");
			...
			ANT_CORE_ASSERT(attachments.back());
		}
