...
namespace Ant{
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL VulkanDebugReportCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
	{
		...
		ANT_CORE_WARN_TAG("Renderer", "VulkanDebugCallback:\n  Object Type: {0}\n  Message: {1}", objectType, pMessage);
	...
	static bool CheckDriverAPIVersionSupport(uint32_t minimumSupportedVersion)
	{
		...
		{
			ANT_CORE_FATAL("Incompatible Vulkan driver version!");
			ANT_CORE_FATAL("  You have {}.{}.{}", VK_API_VERSION_MAJOR(instanceVersion), VK_API_VERSION_MINOR(instanceVersion), VK_API_VERSION_PATCH(instanceVersion));
			ANT_CORE_FATAL("  You need at least {}.{}.{}", VK_API_VERSION_MAJOR(minimumSupportedVersion), VK_API_VERSION_MINOR(minimumSupportedVersion), VK_API_VERSION_PATCH(minimumSupportedVersion));
	...
	{
		ANT_CORE_INFO_TAG("Renderer", "VulkanContext::Create");

		ANT_CORE_ASSERT(glfwVulkanSupported(), "GLFW must support Vulkan!");

		if (!CheckDriverAPIVersionSupport(VK_API_VERSION_1_2))
		{
			MessageBox(nullptr, L"Incompatible Vulkan driver version.\nUpdate your GPU drivers!", L"Ant Error", MB_OK | MB_ICONERROR);
		...
		appInfo.pApplicationName = "Ant";
		appInfo.pEngineName = "Ant";
...
#define VK_KHR_WIN32_SURFACE_EXTENSION_NAME "VK_KHR_win32_surface"
		...
		if (s_Validation)
		{
			...
			ANT_CORE_TRACE_TAG("Renderer", "Vulkan Instance Layers:");
			for (const VkLayerProperties& layer : instanceLayerProperties)
			{
				ANT_CORE_TRACE_TAG("Renderer", "  {0}", layer.layerName);
			...
			{
				ANT_CORE_ERROR_TAG("Renderer", "Validation layer VK_LAYER_KHRONOS_validation not present, validation is disabled");
			}
		}
		...
		if (s_Validation)
		{
			...
			ANT_CORE_ASSERT(vkCreateDebugReportCallbackEXT != NULL, "");
			...
			ANT_CORE_ASSERT(vkCreateDebugUtilsMessengerEXT != NULL, "");
