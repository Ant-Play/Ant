...
namespace Ant{
	...
	void VulkanShader::RT_Reload(const bool forceCompile)
	{
		...
		{
			ANT_CORE_FATAL("Failed to recompile shader!");
		}
#endif
	}
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (uint32_t set = 0; set < m_ReflectionData.ShaderDescriptorSets.size(); set++)
		{
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				ANT_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				ANT_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.SeparateSamplers)
			{
				...
				ANT_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				ANT_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
				ANT_CORE_ASSERT(shaderDescriptorSet.SeparateTextures.find(binding) == shaderDescriptorSet.SeparateTextures.end(), "Binding is already present!");
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				ANT_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				ANT_CORE_ASSERT(shaderDescriptorSet.SeparateTextures.find(binding) == shaderDescriptorSet.SeparateTextures.end(), "Binding is already present!");
				ANT_CORE_ASSERT(shaderDescriptorSet.SeparateSamplers.find(binding) == shaderDescriptorSet.SeparateSamplers.end(), "Binding is already present!");
			...
			ANT_CORE_INFO_TAG("Renderer", "Creating descriptor set {0} with {1} ubo's, {2} ssbo's, {3} samplers, {4} separate textures, {5} separate samplers and {6} storage images", set,
	...
	{
		ANT_CORE_ASSERT(set < m_DescriptorSetLayouts.size());
	...
	{
		ANT_CORE_ASSERT(set < m_ReflectionData.ShaderDescriptorSets.size());
		ANT_CORE_ASSERT(m_ReflectionData.ShaderDescriptorSets[set]);
		...
		{
			ANT_CORE_WARN_TAG("Renderer", "Shader {0} does not contain requested descriptor set {1}", m_Name, name);
			return nullptr;
		}
