...
namespace Ant{
	...
	void VulkanShader::Reflect(VkShaderStageFlagBits shaderStage, const std::vector<uint32_t>& shaderData)
	{
		...
		ANT_CORE_TRACE("===========================");
		ANT_CORE_TRACE(" Vulkan Shader Reflection");
		ANT_CORE_TRACE(" {0}", m_AssetPath);
		ANT_CORE_TRACE("===========================");
		...
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			//ANT_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end());
			...
			else
			{
				...
				{
					ANT_CORE_TRACE("Resizing uniform buffer (binding = {0}, set = {1}) to {2} bytes", binding, descriptorSet, size);
					uniformBuffer->Size = size;
					AllocateUniformBuffer(*uniformBuffer);
				}
			...
			ANT_CORE_TRACE("  {0} ({1}, {2})", name, descriptorSet, binding);
			ANT_CORE_TRACE("  Member Count: {0}", memberCount);
			ANT_CORE_TRACE("  Size: {0}", size);
			ANT_CORE_TRACE("-------------------");
		}

		ANT_CORE_TRACE("Push Constant Buffers:");
		for (const auto& resource : resources.push_constant_buffers)
		{
			...
			ANT_CORE_TRACE("  Name: {0}", bufferName);
			ANT_CORE_TRACE("  Member Count: {0}", memberCount);
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (auto&& [set, shaderDescriptorSet] : m_ShaderDescriptorSets)
		{
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				ANT_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				ANT_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				ANT_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
			...
			ANT_CORE_INFO("Creating descriptor set {0} with {1} ubos, {2} samplers and {3} storage images", set,
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set)
	{
		...
		ANT_CORE_ASSERT(m_TypeCounts.find(set) != m_TypeCounts.end());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set, uint32_t numberOfSets)
	{
		...
		ANT_CORE_ASSERT(poolSizes.find(set) != poolSizes.end());
	...
	{
		ANT_CORE_ASSERT(m_ShaderDescriptorSets.find(set) != m_ShaderDescriptorSets.end());
		if (m_ShaderDescriptorSets.at(set).WriteDescriptorSets.find(name) == m_ShaderDescriptorSets.at(set).WriteDescriptorSets.end())
		{
			ANT_CORE_WARN("Shader {0} does not contain requested descriptor set {1}", m_Name, name);
			return nullptr;
		}
	...
	std::unordered_map<VkShaderStageFlagBits, std::string> VulkanShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			ANT_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute", "Invalid shader type specified");
	...
	{
		ANT_CORE_ASSERT(s_UniformBuffers.find(set) != s_UniformBuffers.end());
		ANT_CORE_ASSERT(s_UniformBuffers.at(set).find(bindingPoint) != s_UniformBuffers.at(set).end());
		ANT_CORE_ASSERT(s_UniformBuffers.at(set).at(bindingPoint));
