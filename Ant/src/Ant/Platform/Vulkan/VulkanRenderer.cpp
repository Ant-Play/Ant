...
namespace Ant{
	...
	{
		s_Data = anew VulkanRendererData();
		...
		QuadVertex* data = anew QuadVertex[4];
	...
	{
		ANT_PROFILE_FUNC();
		ANT_SCOPE_PERF("VulkanRenderer::RT_UpdateMaterialForRendering");
	...
	void VulkanRenderer::RenderStaticMesh(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<StaticMesh> mesh, uint32_t submeshIndex, Ref<MaterialTable> materialTable, Ref<VertexBuffer> transformBuffer, uint32_t transformOffset, uint32_t instanceCount)
	{
		ANT_CORE_VERIFY(mesh);
		ANT_CORE_VERIFY(materialTable);
		...
		Renderer::Submit([renderCommandBuffer, pipeline, uniformBufferSet, storageBufferSet, mesh, submeshIndex, materialTable, transformBuffer, transformOffset, instanceCount]() mutable
			{
				ANT_PROFILE_FUNC("VulkanRenderer::RenderMesh");
				ANT_SCOPE_PERF("VulkanRenderer::RenderMesh");
...
#if 0
	void VulkanRenderer::RenderSubmesh(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Mesh> mesh, uint32_t submeshIndex, Ref<MaterialTable> materialTable, const glm::mat4& transform)
	{
		ANT_CORE_VERIFY(mesh);
		ANT_CORE_VERIFY(materialTable);
		...
		Renderer::Submit([renderCommandBuffer, pipeline, uniformBufferSet, storageBufferSet, mesh, submeshIndex, transform, materialTable]() mutable
			{
				...
				if (mesh->IsRigged())
				{
					auto temp = vulkanPipeline->GetSpecification().Shader.As<VulkanShader>()->AllocateDescriptorSet(2); //  hard coding 2 = animation data.  Yuk.
	...
	void VulkanRenderer::RenderSubmeshInstanced(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Mesh> mesh, uint32_t submeshIndex, Ref<MaterialTable> materialTable, Ref<VertexBuffer> transformBuffer, uint32_t transformOffset, const std::vector<Ref<StorageBuffer>>& boneTransformStorageBuffers, uint32_t boneTransformsOffset, uint32_t instanceCount)
	{
		ANT_CORE_VERIFY(mesh);
		ANT_CORE_VERIFY(materialTable);
	...
	void VulkanRenderer::RenderMeshWithMaterial(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Mesh> mesh, uint32_t submeshIndex, Ref<Material> material, Ref<VertexBuffer> transformBuffer, uint32_t transformOffset, const std::vector<Ref<StorageBuffer>>& boneTransformStorageBuffers, uint32_t boneTransformsOffset, uint32_t instanceCount, Buffer additionalUniforms)
	{
		ANT_CORE_ASSERT(mesh);
		ANT_CORE_ASSERT(mesh->GetMeshSource());
		...
		Renderer::Submit([renderCommandBuffer, pipeline, uniformBufferSet, storageBufferSet, mesh, submeshIndex, vulkanMaterial, transformBuffer, transformOffset, boneTransformStorageBuffers, instanceCount, pushConstantBuffer]() mutable
			{
				ANT_PROFILE_FUNC("VulkanRenderer::RenderMeshWithMaterial");
				ANT_SCOPE_PERF("VulkanRenderer::RenderMeshWithMaterial");
				...
				if (submesh.IsRigged)
				{
					...
					auto temp = vulkanPipeline->GetSpecification().Shader.As<VulkanShader>()->AllocateDescriptorSet(1); //  hard coding 1 = animation data.  Yuk.
	...
	void VulkanRenderer::RenderStaticMeshWithMaterial(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<StaticMesh> staticMesh, uint32_t submeshIndex, Ref<Material> material, Ref<VertexBuffer> transformBuffer, uint32_t transformOffset, uint32_t instanceCount, Buffer additionalUniforms /*= Buffer()*/)
	{
		ANT_CORE_ASSERT(staticMesh);
		ANT_CORE_ASSERT(staticMesh->GetMeshSource());
	...
	void VulkanRenderer::RenderQuad(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material, const glm::mat4& transform)
	{
		...
		Renderer::Submit([renderCommandBuffer, pipeline, uniformBufferSet, storageBufferSet, vulkanMaterial, transform]() mutable
			{
				ANT_PROFILE_FUNC("VulkanRenderer::RenderQuad");
	...
	void VulkanRenderer::RenderGeometry(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material, Ref<VertexBuffer> vertexBuffer, Ref<IndexBuffer> indexBuffer, const glm::mat4& transform, uint32_t indexCount /*= 0*/)
	{
		...
		Renderer::Submit([renderCommandBuffer, pipeline, uniformBufferSet, vulkanMaterial, vertexBuffer, indexBuffer, transform, indexCount]() mutable
			{
				ANT_PROFILE_FUNC("VulkanRenderer::RenderGeometry");
		...
		Renderer::Submit([uniformBuffer, set]()
			{
				...
				ANT_CORE_ASSERT(set == 1); // Currently we only bind to Renderer-maintaned UBs, which are in descriptor set 1
				...
				ANT_CORE_WARN("VulkanRenderer - Updating descriptor set (VulkanRenderer::SetUniformBuffer)");
				VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
	...
	void VulkanRenderer::DispatchComputeShader(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<PipelineCompute> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material, const glm::uvec3& workGroups, Buffer additionalUniforms)
	{
	...
	void VulkanRenderer::LightCulling(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<PipelineCompute> pipelineCompute, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material, const glm::uvec3& workGroups)
	{
		auto vulkanMaterial = material.As<VulkanMaterial>();
	...
	void VulkanRenderer::SubmitFullscreenQuad(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<Material> material)
	{
		Ref<VulkanMaterial> vulkanMaterial = material.As<VulkanMaterial>();
		Renderer::Submit([renderCommandBuffer, pipeline, uniformBufferSet, storageBufferSet, vulkanMaterial]() mutable
			{
				ANT_PROFILE_FUNC("VulkanRenderer::SubmitFullscreenQuad");
	...
	void VulkanRenderer::SetSceneEnvironment(Ref<SceneRenderer> sceneRenderer, Ref<Environment> environment, Ref<Image2D> shadow, Ref<Image2D> spotShadow)
	{
		...
		Renderer::Submit([sceneRenderer, environment, shadow, spotShadow]() mutable
			{
				ANT_PROFILE_FUNC("VulkanRenderer::SetSceneEnvironment");

				const auto shader = Renderer::GetShaderLibrary()->Get("AntPBR_Static");
	...
	void VulkanRenderer::BeginFrame()
	{
		Renderer::Submit([]()
			{
				ANT_PROFILE_FUNC("VulkanRenderer::BeginFrame");
	...
	void VulkanRenderer::BeginRenderPass(Ref<RenderCommandBuffer> renderCommandBuffer, const Ref<RenderPass>& renderPass, bool explicitClear)
	{
		Renderer::Submit([renderCommandBuffer, renderPass, explicitClear]()
			{
				ANT_PROFILE_SCOPE_DYNAMIC(fmt::format("VulkanRenderer::BeginRenderPass ({})", renderPass->GetSpecification().DebugName).c_str());
				ANT_CORE_TRACE_TAG("Renderer", "BeginRenderPass - {}", renderPass->GetSpecification().DebugName);
				...
				if (explicitClear)
				{
					...
					ANT_CORE_ASSERT(clearValues.size() == totalAttachmentCount);
	...
	void VulkanRenderer::EndRenderPass(Ref<RenderCommandBuffer> renderCommandBuffer)
	{
		Renderer::Submit([renderCommandBuffer]()
			{
				ANT_PROFILE_FUNC("VulkanRenderer::EndRenderPass");
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> VulkanRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		ANT_CORE_ASSERT(envEquirect->GetFormat() == ImageFormat::RGBA32F, "Texture is not HDR!");
