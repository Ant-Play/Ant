...
namespace Ant {
	...
	{
		ANT_CORE_ERROR("GLFW Error ({0}); {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		ANT_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (s_GLFWWindowCount == 0)
		{
			ANT_PROFILE_SCOPE("glfwInit");
			int success = glfwInit();
			ANT_CORE_ASSERT(success, "Could not initialize GLFW");
			glfwSetErrorCallback(GLFWErrorCallback);
		}

		{
			ANT_PROFILE_SCOPE("glfwCreateWindow");
