...
namespace Ant {
	...
	{
		ANT_CORE_ERROR_TAG("GLFW", "GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init()
	{
		...
		ANT_CORE_INFO_TAG("GLFW", "Creating window {0} ({1}, {2})", m_Specification.Title, m_Specification.Width, m_Specification.Height);
		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			ANT_CORE_ASSERT(success, "Could not intialize GLFW!");
		...
		if (isRawMouseMotionSupported)
			...
			ANT_CORE_WARN_TAG("Platform", "Raw mouse motion not supported.");
