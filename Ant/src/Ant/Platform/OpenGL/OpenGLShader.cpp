...
//#if UNIFORM_LOGGING
//#define ANT_LOG_UNIFORM(...) ANT_CORE_WARN(__VA_ARGS__)
//#else
//#define ANT_LOG_UNIFORM
//#endif
//
...
//			ANT_CORE_ERROR("Shader compilation failed ({0}):\n{1}", m_AssetPath, &infoLog[0]);
//
...
//				if (location == -1)
//					ANT_CORE_WARN("{0}: could not find uniform location {0}", name);
//
...
//
//		ANT_CORE_TRACE("OpenGLShader::Reflect - {0}", m_AssetPath);
//		ANT_CORE_TRACE("   {0} Uniform Buffers", res.uniform_buffers.size());
//		ANT_CORE_TRACE("   {0} Resources", res.sampled_images.size());
//
...
//
//				ANT_CORE_TRACE("Created Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
//
...
//				ANT_CORE_ASSERT(buffer.Name == resource.name); // Must be the same buffer
...
//					glBindBufferBase(GL_UNIFORM_BUFFER, buffer.BindingPoint, buffer.RendererID);
//
//					ANT_CORE_TRACE("Resized Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
//				}
//			}
//		}
//
...
//		if (result == -1)
//			ANT_CORE_WARN("Could not find uniform '{0}' in shader", name);
//
//		return result;
//	}
//
...
//		}
//
//		ANT_CORE_ASSERT(uniformBuffer);
//		ANT_CORE_ASSERT(uniformBuffer->Size >= size);
...
//		Renderer::Submit([=]()
//			{
//				ANT_CORE_ASSERT(m_UniformLocations.find(fullname) != m_UniformLocations.end());
...
//			glUniform1f(location, value);
//		else
//			ANT_LOG_UNIFORM("Uniform '{0}' not found!", name);
//	}
//
