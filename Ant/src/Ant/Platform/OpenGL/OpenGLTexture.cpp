...
namespace Ant {
	...
	static GLenum AntToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
		...
		Renderer::Submit([instance]() mutable
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, AntToOpenGLTextureFormat(instance->m_Format), instance->m_Width, instance->m_Height, 0, AntToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		{
			ANT_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
		...
		{
			ANT_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = stbi_load(path.c_str(), &width, &height, &channels, srgb ? STBI_rgb : STBI_rgb_alpha);
			ANT_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
		...
		Renderer::Submit([instance, srgb]() mutable
			{
				...
				else
				{
					...
					GLenum internalFormat = AntToOpenGLTextureFormat(instance->m_Format);
					GLenum format = srgb ? GL_SRGB8 : (instance->m_IsHDR ? GL_RGB : AntToOpenGLTextureFormat(instance->m_Format)); // HDR = GL_RGB for now
	...
	void OpenGLTexture2D::Unlock()
	{
		...
		Renderer::Submit([instance]() {
			glTextureSubImage2D(instance->m_RendererID, 0, 0, 0, instance->m_Width, instance->m_Height, AntToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, instance->m_ImageData.Data);
			});
	}

	void OpenGLTexture2D::Resize(uint32_t width, uint32_t height)
	{
		ANT_CORE_ASSERT(m_Locked, "Texture must be locked!");
		...
		Renderer::Submit([instance, levels]() mutable
			{
				...
				glTextureStorage2D(instance->m_RendererID, levels, AntToOpenGLTextureFormat(instance->m_Format), instance->m_Width, instance->m_Height);
				glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MIN_FILTER, levels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
				glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		: m_FilePath(path)
	{
		...
		ANT_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		Renderer::Submit([instance, faceWidth, faceHeight, faces]() mutable
			{
				...
				auto format = AntToOpenGLTextureFormat(instance->m_Format);
