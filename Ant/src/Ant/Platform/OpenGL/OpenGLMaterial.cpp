...
//		const auto& shaderBuffers = m_Shader->GetShaderBuffers();
//
//		ANT_CORE_ASSERT(shaderBuffers.size() <= 1, "We currently only support ONE material buffer!");
//
//		if (shaderBuffers.size() > 0)
//		{
...
//		uint32_t slot = decl->GetRegister();
//		ANT_CORE_ASSERT(slot < m_Texture2Ds.size(), "Texture slot is invalid");
...
//					{
//						ANT_CORE_ASSERT(texture->GetType() == TextureType::TextureCube);
