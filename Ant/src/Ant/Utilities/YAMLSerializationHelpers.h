...
namespace YAML {
	...
	template<>
	struct convert<Ant::AssetHandle>
	{
		static Node encode(const Ant::AssetHandle& rhs)
		{
			Node node;
		...
		static bool decode(const Node& node, Ant::AssetHandle& rhs)
		{
	...
	struct convert<choc::value::Value>
	{
		static Node encode(const choc::value::ValueView& rhs)
		{
			...
			else	if (rhs.isArray() || rhs.isVector())	{ out["Type"] = "Array";	out["Value"] = encodeArrayOrVector(rhs); out.SetStyle(EmitterStyle::Block); }
	...
	private:
		static bool decodeArrayOrVector(const Node& node, choc::value::Value& rhs)
		{
			...
			// TODO: for not we don't distinguish types of the array,
