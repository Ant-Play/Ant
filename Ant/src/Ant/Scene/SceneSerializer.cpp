...
#include "Ant/Physics/PhysicsLayer.h"
...
namespace Ant{
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		ANT_CORE_INFO("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				ANT_CORE_INFO("Deserialized entity with ID = {0}, name = {1}", uuid, name);
				...
				if (transformComponent)
				{
					...
					ANT_CORE_INFO("  Entity Transform:");
					ANT_CORE_INFO("    Translation: {0}, {1}, {2}", transform.Translation.x, transform.Translation.y, transform.Translation.z);
					ANT_CORE_INFO("    Rotation: {0}, {1}, {2}", transform.Rotation.x, transform.Rotation.y, transform.Rotation.z);
					ANT_CORE_INFO("    Scale: {0}, {1}, {2}", transform.Scale.x, transform.Scale.y, transform.Scale.z);
				}
				...
				if (scriptComponent)
				{
					...
					ANT_CORE_INFO("  Script Module: {0}", moduleName);
						...
						if (storedFields)
						{
							for (auto field : storedFields)
							{
								...
								switch (type)
								{
									...
									{
										ANT_CORE_ASSERT(false, "Unimplemented");
										break;
									}
									case FieldType::Vec2:
									{
				...
				if (skyLightComponent)
				{
					...
					else
					{
						...
						if (filepath.empty())
						{
							ANT_CORE_ERROR("Tried to load non-existent environment map in Entity: {0}", deserializedEntity.GetUUID());
						}
						else
						{
							ANT_CORE_ERROR("Tried to load invalid environment map '{0}' in Entity {1}", filepath, deserializedEntity.GetUUID());
						}
					}
				...
				if (boxColliderComponent)
				{
					...
					if (material)
					{
						...
						{
							ANT_CORE_ERROR("Tried to load invalid Physics Material in Entity {0}", deserializedEntity.GetUUID());
						}
					}
