...
namespace Ant{
	...
	void SceneSerializer::SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		{
#define ANT_SERIALIZE_PROPERTY(propName, propVal, outputData) outputData << YAML::Key << #propName << YAML::Value << propVal

			out << YAML::Key << "AudioComponent";
			out << YAML::BeginMap; // AudioComponent
			auto& audioComponent = entity.GetComponent<AudioComponent>();

			ANT_SERIALIZE_PROPERTY(StartEvent, audioComponent.StartEvent, out);
			ANT_SERIALIZE_PROPERTY(StartCommandID, audioComponent.StartCommandID, out);

			ANT_SERIALIZE_PROPERTY(PlayOnAwake, audioComponent.bPlayOnAwake, out);
			ANT_SERIALIZE_PROPERTY(StopIfEntityDestroyed, audioComponent.bStopWhenEntityDestroyed, out);
			ANT_SERIALIZE_PROPERTY(VolumeMultiplier, audioComponent.VolumeMultiplier, out);
			ANT_SERIALIZE_PROPERTY(PitchMultiplier, audioComponent.PitchMultiplier, out);
			ANT_SERIALIZE_PROPERTY(AutoDestroy, audioComponent.bAutoDestroy, out);
		...
		if (entity.HasComponent<AudioListenerComponent>())
		{
			...
			ANT_SERIALIZE_PROPERTY(Active, audioListenerComponent.Active, out);
			ANT_SERIALIZE_PROPERTY(ConeInnerAngle, audioListenerComponent.ConeInnerAngleInRadians, out);
			ANT_SERIALIZE_PROPERTY(ConeOuterAngle, audioListenerComponent.ConeOuterAngleInRadians, out);
			ANT_SERIALIZE_PROPERTY(ConeOuterGain, audioListenerComponent.ConeOuterGain, out);
			out << YAML::EndMap; // AudioComponent

#undef ANT_SERIALIZE_PROPERTY
		}
	...
	bool SceneSerializer::DeserializeFromYAML(const std::string& yamlString)
	{
		...
		ANT_CORE_INFO_TAG("AssetManager", "Deserializing scene '{0}'", sceneName);
	...
	void SceneSerializer::DeserializeEntities(YAML::Node& entitiesNode, Ref<Scene> scene)
	{
		for (auto entity : entitiesNode)
		{
			...
			//ANT_CORE_INFO("Deserialized Entity '{0}' with ID '{1}'", name, uuid);
			...
			if (transformComponent)
			{
				...
				// Early versions of Ant saved rotations as quaternions
				...
				{
					ANT_CORE_ASSERT(rotationNode.size() == 3);
			...
			if (scriptComponent)
			{
				...
				if (scriptAssetHandle == 0 && !moduleName.empty())
					scriptAssetHandle = ANT_SCRIPT_CLASS_ID(moduleName);
					...
					if (sc.FieldIDs.size() > 0)
					{
						...
						if (storedFields)
						{
							for (auto field : storedFields)
							{
								...
								if (storage == nullptr)
								{
									ANT_CONSOLE_LOG_WARN("Serialized C# field {0} doesn't exist in script cache! This could be because the script field no longer exists or because it's been renamed.", name);
								}
								else
								{
				...
				{
					ANT_CORE_ERROR("Failed to deserialize ScriptComponent for entity '{0}'! Couldn't find a valid ModuleName or ClassHandle field/value!", deserializedEntity.Name());
				}
			}
			...
			if (meshComponent)
			{
				...
				if (AssetManager::IsAssetHandleValid(assetHandle))
				{
					...
					else if (type == AssetType::MeshSource)
					{
						...
						std::string filename = fmt::format("{0}.amesh", meshPath.stem().string());
				...
				/*else
				{
					...
					if (filepath.empty())
					{
						ANT_CORE_ERROR("Tried to load non-existent mesh in Entity: {0}", deserializedEntity.GetUUID());
					}
					else
					{
						ANT_CORE_ERROR("Tried to load invalid mesh '{0}' in Entity {1}", filepath, deserializedEntity.GetUUID());
			...
			if (skyLightComponent)
			{
				...
				//{
				//	ANT_CORE_ERROR("Tried to load invalid environment map in Entity {0}", deserializedEntity.GetUUID());
				//}
			}
			...
			if (meshColliderComponent)
			{
				...
				else
				{
					...
					if (AssetManager::IsAssetHandleValid(component.ColliderAsset))
						...
						ANT_CORE_WARN("MeshColliderComponent in use without valid mesh!");
			...
			if (audioComponent)
			{
#define ANT_DESERIALIZE_PROPERTY(propName, destination, inputData, defaultValue) destination = inputData[#propName] ? inputData[#propName].as<decltype(defaultValue)>() : defaultValue

				auto& component = deserializedEntity.AddComponent<AudioComponent>();

				ANT_DESERIALIZE_PROPERTY(StartEvent, component.StartEvent, audioComponent, std::string(""));
				...
				ANT_DESERIALIZE_PROPERTY(PlayOnAwake, component.bPlayOnAwake, audioComponent, false);
				ANT_DESERIALIZE_PROPERTY(StopIfEntityDestroyed, component.bStopWhenEntityDestroyed, audioComponent, true);
				ANT_DESERIALIZE_PROPERTY(VolumeMultiplier, component.VolumeMultiplier, audioComponent, 1.0f);
				ANT_DESERIALIZE_PROPERTY(PitchMultiplier, component.PitchMultiplier, audioComponent, 1.0f);
				ANT_DESERIALIZE_PROPERTY(AutoDestroy, component.bAutoDestroy, audioComponent, false);
			}
...
#undef ANT_DESERIALIZE_PROPERTY
			}
		}
