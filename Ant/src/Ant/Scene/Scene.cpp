...
namespace Ant {
	...
	void Scene::OnUpdateRuntime(Timestep ts)
	{
		...
		{
			ANT_PROFILE_FUNC("Box2DWorld::Step");
		...
		if (m_ShouldSimulate)
		{
			...
			if (m_IsPlaying)
			{
				ANT_PROFILE_FUNC("Scene::OnUpdate - C# OnUpdate");
		...
		{	//--- Update Audio Listener ---
			...
			ANT_PROFILE_FUNC("Scene::OnUpdate - Update Audio Listener");
		...
		{	//--- Update Audio Components ---
			...
			ANT_PROFILE_FUNC("Scene::OnUpdate - Update Audio Components");
	...
	void Scene::OnRenderRuntime(Ref<SceneRenderer> renderer, Timestep ts)
	{
		...
		// Render Static Meshes
		{
			...
			{
				ANT_PROFILE_FUNC("Scene-SubmitStaticMesh");
		...
		// Render Dynamic Meshes
		{
			...
			for (auto entity : view)
			{
				ANT_PROFILE_FUNC("Scene-SubmitDynamicMesh");
	...
	void Scene::OnAudioComponentConstruct(entt::registry& registry, entt::entity entity)
	{
		...
		ANT_CORE_ASSERT(m_EntityIDMap.find(entityID) != m_EntityIDMap.end());
	...
	Entity Scene::GetMainCameraEntity()
	{
		...
		for (auto entity : view)
		{
			...
			if (comp.Primary)
			{
				ANT_CORE_ASSERT(comp.Camera.GetOrthographicSize() || comp.Camera.GetDegPerspectiveVerticalFOV(), "Camera is not fully initialized");
				return { entity, this };
			}
		}
		return {};
	...
	void Scene::SubmitToDestroyEntity(Entity entity)
	{
		...
		if (!isValid)
		{
			ANT_CORE_WARN_TAG("Scene", "Trying to destroy invalid entity! entt={0}", (uint32_t)entity);
			return;
		}
	...
	Entity Scene::DuplicateEntity(Entity entity)
	{
		...
		if (entity.HasComponent<PrefabComponent>())
		{
			...
			ANT_CORE_VERIFY(AssetManager::IsAssetHandleValid(prefabID));
		...
		ANT_CORE_ASSERT(foundAll, "At least one component was not duplicated - have you added a new component type and not dealt with it here?");
#endif
	...
	Entity Scene::CreatePrefabEntity(Entity entity, Entity parent, const glm::vec3* translation, const glm::vec3* rotation, const glm::vec3* scale)
	{
		...
		ANT_CORE_VERIFY(entity.HasComponent<PrefabComponent>());
	...
	{
		ANT_PROFILE_FUNC();
		ANT_CORE_VERIFY(m_EntityIDMap.find(id) != m_EntityIDMap.end(), "Invalid entity ID or entity doesn't exist in scene!");
	...
	void Scene::OnSceneTransition(const std::string& scene)
	{
		...
		if (!m_OnSceneTransitionCallback)
		{
			ANT_CORE_WARN("Cannot transition scene - no callback set!");
		}
	}
	...
	std::unordered_set<AssetHandle> Scene::GetAssetList()
	{
		...
		//ANT_CORE_WARN("{} assets ({} missing)", assetList.size(), missingAssets.size());
		return assetList;
	}
}
