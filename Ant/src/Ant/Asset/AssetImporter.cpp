...
#include "Ant/Animation/AnimationAssetSerializer.h"
...
namespace Ant{
	...
	void AssetImporter::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset)
	{
		...
		{
			ANT_CORE_WARN("There's currently no importer for assets of type {0}", metadata.FilePath.stem().string());
			return;
		}
	...
	bool AssetImporter::TryLoadData(const AssetMetadata& metadata, Ref<Asset>& asset)
	{
		...
		// ANT_CORE_TRACE("AssetImporter::TryLoadData - {}", metadata.FilePath);
