...
#include "Ant/Audio/SoundBank.h"
#include "Ant/Audio/ResourceManager.h"
...
#include "Ant/Serialization/TextureRuntimeSerializer.h"
...
namespace Ant{
	...
	{
		ANT_CORE_WARN("TextureSerializer::DeserializeFromAssetPack");
	...
	std::string MaterialAssetSerializer::SerializeToYAML(Ref<MaterialAsset> materialAsset) const
	{
		...
		out << YAML::Key << "Material" << YAML::Value;
		{
			...
			ANT_SERIALIZE_PROPERTY(Transparent, transparent, out);

			ANT_SERIALIZE_PROPERTY(AlbedoColor, materialAsset->GetAlbedoColor(), out);
			ANT_SERIALIZE_PROPERTY(Emission, materialAsset->GetEmission(), out);
			if (!transparent)
			{
				ANT_SERIALIZE_PROPERTY(UseNormalMap, materialAsset->IsUsingNormalMap(), out);
				ANT_SERIALIZE_PROPERTY(Metalness, materialAsset->GetMetalness(), out);
				ANT_SERIALIZE_PROPERTY(Roughness, materialAsset->GetRoughness(), out);
			}
			else
			{
				ANT_SERIALIZE_PROPERTY(Transparency, materialAsset->GetTransparency(), out);
			}

			{
				...
				ANT_SERIALIZE_PROPERTY(AlbedoMap, albedoMapHandle, out);
			}
			if (!transparent)
			{
				{
					...
					ANT_SERIALIZE_PROPERTY(NormalMap, normalMapHandle, out);
				}
				{
					...
					ANT_SERIALIZE_PROPERTY(MetalnessMap, metalnessMapHandle, out);
				}
				{
					...
					ANT_SERIALIZE_PROPERTY(RoughnessMap, roughnessMapHandle, out);
				}
			}

			ANT_SERIALIZE_PROPERTY(MaterialFlags, materialAsset->GetMaterial()->GetFlags(), out);
	...
	bool MaterialAssetSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<MaterialAsset>& targetMaterialAsset) const
	{
		...
		ANT_DESERIALIZE_PROPERTY(Transparent, transparent, materialNode, false);

		targetMaterialAsset = Ref<MaterialAsset>::Create(transparent);

		ANT_DESERIALIZE_PROPERTY(AlbedoColor, targetMaterialAsset->GetAlbedoColor(), materialNode, glm::vec3(0.8f));
		ANT_DESERIALIZE_PROPERTY(Emission, targetMaterialAsset->GetEmission(), materialNode, 0.0f);

		if (!transparent)
		{
			targetMaterialAsset->SetUseNormalMap(materialNode["UseNormalMap"] ? materialNode["UseNormalMap"].as<bool>() : false);
			ANT_DESERIALIZE_PROPERTY(Metalness, targetMaterialAsset->GetMetalness(), materialNode, 0.0f);
			ANT_DESERIALIZE_PROPERTY(Roughness, targetMaterialAsset->GetRoughness(), materialNode, 0.5f);
		}
		else
		{
			ANT_DESERIALIZE_PROPERTY(Transparency, targetMaterialAsset->GetTransparency(), materialNode, 1.0f);
		}

		AssetHandle albedoMap, normalMap, metalnessMap, roughnessMap;
		ANT_DESERIALIZE_PROPERTY(AlbedoMap, albedoMap, materialNode, (AssetHandle)0);
		if (!transparent)
		{
			ANT_DESERIALIZE_PROPERTY(NormalMap, normalMap, materialNode, (AssetHandle)0);
			ANT_DESERIALIZE_PROPERTY(MetalnessMap, metalnessMap, materialNode, (AssetHandle)0);
			ANT_DESERIALIZE_PROPERTY(RoughnessMap, roughnessMap, materialNode, (AssetHandle)0);
		}
		if (albedoMap)
		{
		...
		ANT_DESERIALIZE_PROPERTY(MaterialFlags, roughnessMap, materialNode, (AssetHandle)0);
	...
	void PhysicsMaterialSerializer::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset) const
	{
		...
		std::string yamlString = SerializeToYAML(physicsMaterial);
	...
	bool PhysicsMaterialSerializer::TryLoadData(const AssetMetadata& metadata, Ref<Asset>& asset) const
	{
		...
		bool success = DeserializeFromYAML(strStream.str(), physicsMaterial);
		if (!success)
			return false;
	...
	bool PhysicsMaterialSerializer::SerializeToAssetPack(AssetHandle handle, FileStreamWriter& stream, AssetSerializationInfo& outInfo) const
	{
		Ref<PhysicsMaterial> physicsMaterial = AssetManager::GetAsset<PhysicsMaterial>(handle);
	...
	Ref<Asset> PhysicsMaterialSerializer::DeserializeFromAssetPack(FileStreamReader& stream, const AssetPackFile::AssetInfo& assetInfo) const
	{
		stream.SetStreamPosition(assetInfo.PackedOffset);
		std::string yamlString;
		stream.ReadString(yamlString);
		...
		Ref<PhysicsMaterial> physicsMaterial;
		bool result = DeserializeFromYAML(yamlString, physicsMaterial);
		if (!result)
			return nullptr;
	...
	std::string PhysicsMaterialSerializer::SerializeToYAML(Ref<PhysicsMaterial> physicsMaterial) const
	{
		YAML::Emitter out;
	...
	bool PhysicsMaterialSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<PhysicsMaterial>& targetPhysicsMaterial) const
	{
		...
		targetPhysicsMaterial = Ref<PhysicsMaterial>::Create(staticFriction, dynamicFriction, bounciness);
	...
	bool SoundConfigSerializer::TryLoadData(const AssetMetadata& metadata, Ref<Asset>& asset) const
	{
		...
		// TODO: Maybe we should make a cleaner intefrace to reuse with other asset types as well?
	...
	std::string SoundConfigSerializer::SerializeToYAML(Ref<SoundConfig> soundConfig) const
	{
		...
		{
			ANT_SERIALIZE_PROPERTY(AssetID, soundConfig->DataSourceAsset, out);
		}

		ANT_SERIALIZE_PROPERTY(IsLooping, (bool)soundConfig->bLooping, out);
		ANT_SERIALIZE_PROPERTY(VolumeMultiplier, soundConfig->VolumeMultiplier, out);
		ANT_SERIALIZE_PROPERTY(PitchMultiplier, soundConfig->PitchMultiplier, out);
		ANT_SERIALIZE_PROPERTY(MasterReverbSend, soundConfig->MasterReverbSend, out);
		ANT_SERIALIZE_PROPERTY(LPFilterValue, soundConfig->LPFilterValue, out);
		ANT_SERIALIZE_PROPERTY(HPFilterValue, soundConfig->HPFilterValue, out);
		...
		ANT_SERIALIZE_PROPERTY(Enabled, soundConfig->bSpatializationEnabled, out);
		ANT_SERIALIZE_PROPERTY(AttenuationModel, (int)spatialConfig->AttenuationMod, out);
		ANT_SERIALIZE_PROPERTY(MinGain, spatialConfig->MinGain, out);
		ANT_SERIALIZE_PROPERTY(MaxGain, spatialConfig->MaxGain, out);
		ANT_SERIALIZE_PROPERTY(MinDistance, spatialConfig->MinDistance, out);
		ANT_SERIALIZE_PROPERTY(MaxDistance, spatialConfig->MaxDistance, out);
		ANT_SERIALIZE_PROPERTY(ConeInnerAngle, spatialConfig->ConeInnerAngleInRadians, out);
		ANT_SERIALIZE_PROPERTY(ConeOuterAngle, spatialConfig->ConeOuterAngleInRadians, out);
		ANT_SERIALIZE_PROPERTY(ConeOuterGain, spatialConfig->ConeOuterGain, out);
		ANT_SERIALIZE_PROPERTY(DopplerFactor, spatialConfig->DopplerFactor, out);
		ANT_SERIALIZE_PROPERTY(Rollor, spatialConfig->Rolloff, out);
		ANT_SERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->bAirAbsorptionEnabled, out);
		ANT_SERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->bSpreadFromSourceSize, out);
		ANT_SERIALIZE_PROPERTY(SourceSize, spatialConfig->SourceSize, out);
		ANT_SERIALIZE_PROPERTY(Spread, spatialConfig->Spread, out);
		ANT_SERIALIZE_PROPERTY(Focus, spatialConfig->Focus, out);
	...
	bool SoundConfigSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<SoundConfig> targetSoundConfig) const
	{
		...
		if (Application::IsRuntime())
		{
			...
			if (type == AssetType::Audio)
			{
				...
				ANT_CORE_VERIFY(soundBank, "SoundBank is not loaded!");
		...
		else
		{
			if (Application::IsRuntime())
			{
				ANT_CORE_ERROR("Tried to load invalid audio source asset (UUID {0}) in SoundConfig: {1}",
		...
		ANT_DESERIALIZE_PROPERTY(IsLooping, targetSoundConfig->bLooping, data, false);
		ANT_DESERIALIZE_PROPERTY(VolumeMultiplier, targetSoundConfig->VolumeMultiplier, data, 1.0f);
		ANT_DESERIALIZE_PROPERTY(PitchMultiplier, targetSoundConfig->PitchMultiplier, data, 1.0f);
		ANT_DESERIALIZE_PROPERTY(MasterReverbSend, targetSoundConfig->MasterReverbSend, data, 0.0f);
		ANT_DESERIALIZE_PROPERTY(LPFilterValue, targetSoundConfig->LPFilterValue, data, 20000.0f);
		ANT_DESERIALIZE_PROPERTY(HPFilterValue, targetSoundConfig->HPFilterValue, data, 0.0f);
		...
		if (spConfigData)
		{
			...
			ANT_DESERIALIZE_PROPERTY(Enabled, targetSoundConfig->bSpatializationEnabled, spConfigData, false);
			...
			ANT_DESERIALIZE_PROPERTY(MinGain, spatialConfig->MinGain, spConfigData, 0.0f);
			ANT_DESERIALIZE_PROPERTY(MaxGain, spatialConfig->MaxGain, spConfigData, 1.0f);
			ANT_DESERIALIZE_PROPERTY(MinDistance, spatialConfig->MinDistance, spConfigData, 1.0f);
			ANT_DESERIALIZE_PROPERTY(MaxDistance, spatialConfig->MaxDistance, spConfigData, 1000.0f);
			ANT_DESERIALIZE_PROPERTY(ConeInnerAngle, spatialConfig->ConeInnerAngleInRadians, spConfigData, 6.283185f);
			ANT_DESERIALIZE_PROPERTY(ConeOuterAngle, spatialConfig->ConeOuterAngleInRadians, spConfigData, 6.283185f);
			ANT_DESERIALIZE_PROPERTY(ConeOuterGain, spatialConfig->ConeOuterGain, spConfigData, 0.0f);
			ANT_DESERIALIZE_PROPERTY(DopplerFactor, spatialConfig->DopplerFactor, spConfigData, 1.0f);
			ANT_DESERIALIZE_PROPERTY(Rollor, spatialConfig->Rolloff, spConfigData, 1.0f);
			ANT_DESERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->bAirAbsorptionEnabled, spConfigData, true);
			ANT_DESERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->bSpreadFromSourceSize, spConfigData, true);
			ANT_DESERIALIZE_PROPERTY(SourceSize, spatialConfig->SourceSize, spConfigData, 1.0f);
			ANT_DESERIALIZE_PROPERTY(Spread, spatialConfig->Spread, spConfigData, 1.0f);
			ANT_DESERIALIZE_PROPERTY(Focus, spatialConfig->Focus, spConfigData, 1.0f);
		}
	...
	void ScriptFileSerializer::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset) const
	{
		...
		ANT_CORE_VERIFY(stream.is_open());

		std::ifstream templateStream("Resources/Templates/NewClassTemplate.cs");
		ANT_CORE_VERIFY(templateStream.is_open());
