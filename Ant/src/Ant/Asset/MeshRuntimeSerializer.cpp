...
namespace Ant{
	...
	bool MeshRuntimeSerializer::SerializeToAssetPack(AssetHandle handle, FileStreamWriter& stream, AssetSerializationInfo& outInfo)
	{
		...
		if (hasMaterials)
		{
			...
			for (size_t i = 0; i < meshMaterials.size(); i++)
			{
				...
				material.MetalnessTexture = meshSourceMaterial->GetTexture2D("u_MetalnessTexture")->Handle;
				material.RoughnessTexture = meshSourceMaterial->GetTexture2D("u_RoughnessTexture")->Handle;
			}
		...
		if (hasAnimation || hasSkeleton)
		{
			...
			//ANT_CORE_ASSERT(meshSource->HasSkeleton());
	...
	Ref<Asset> MeshRuntimeSerializer::DeserializeFromAssetPack(FileStreamReader& stream, const AssetPackFile::AssetInfo& assetInfo)
	{
		...
		bool validHeader = memcmp(file.Header.HEADER, "ANTMS", 5) == 0;
