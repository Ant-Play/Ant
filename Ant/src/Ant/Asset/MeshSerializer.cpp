...
namespace Ant{
	...
	void MeshSerializer::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset) const
	{
		...
		ANT_CORE_WARN("Serializing to {0}", serializePath.string());
		std::ofstream fout(serializePath);

		if (!fout.is_open())
		{
			ANT_CORE_ERROR("Failed to serialize mesh to file '{0}'", serializePath);
			ANT_CORE_ASSERT(false, "GetLastError() = {0}", GetLastError());
			return;
		}
	...
	{
		// TODO: this needs to open up a Ant Mesh file and make sure
