...
#define ANT_ASSETREGISTRY_LOG 0
#if ANT_ASSETREGISTRY_LOG
#define ASSET_LOG(...) ANT_CORE_TRACE_TAG("ASSET", __VA_ARGS__)
#else 
#define ASSET_LOG(...)
#endif
	...
	const AssetMetadata& AssetRegistry::Get(const AssetHandle handle) const
	{
		...
		ANT_CORE_ASSERT(m_AssetRegistry.find(handle) != m_AssetRegistry.end());
