...
#include "Ant/Animation/AnimationImporterAssimp.h"
...
#define ANT_MESH_ERROR(...)
	...
	namespace Utils {
		...
		{
			ANT_MESH_LOG("{0:^{1}}{2} {{", "", depth * 3, node->mName.C_Str());
			++depth;
			...
			ANT_MESH_LOG("{0:^{1}}translation: ({2:6.2f}, {3:6.2f}, {4:6.2f})", "", depth * 3, translation.x, translation.y, translation.z);
			ANT_MESH_LOG("{0:^{1}}rotation:    ({2:6.2f}, {3:6.2f}, {4:6.2f})", "", depth * 3, rotation.x, rotation.y, rotation.z);
			ANT_MESH_LOG("{0:^{1}}scale:       ({2:6.2f}, {3:6.2f}, {4:6.2f})", "", depth * 3, scale.x, scale.y, scale.z);
			for (uint32_t i = 0; i < node->mNumChildren; ++i)
			{
				PrintNode(node->mChildren[i], depth);
			}
			--depth;
			ANT_MESH_LOG("{0:^{1}}}}", "", depth * 3);
		}
#endif
	...
	Ref<MeshSource> AssimpMeshImporter::ImportToMeshSource()
	{
		...
		ANT_CORE_INFO_TAG("Mesh", "Loading mesh: {0}", m_Path.string());
		...
		{
			ANT_CORE_ERROR_TAG("Mesh", "Failed to load mesh file: {0}", m_Path.string());
			...
			for (uint32_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				if (mesh->mNumBones > 0)
				{
					...
					for (uint32_t i = 0; i < mesh->mNumBones; i++)
					{
						...
						{
							ANT_CORE_ERROR_TAG("Animation", "Could not find mesh bone '{}' in skeleton!", bone->mName.C_Str());
						}
		...
		{
			ANT_MESH_LOG("---- Materials - {0} ----", filename);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				Ant::Ref<Material> mi = Material::Create(Renderer::GetShaderLibrary()->Get("AntPBR_Static"), aiMaterialName.data);
				...
				if (hasAlbedoMap)
				{
					...
					{
						ANT_CORE_ERROR_TAG("Mesh", "Could not load texture: {0}", aiTexPath.C_Str());
						fallback = true;
					}
				}
				...
				if (hasNormalMap)
				{
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						ANT_MESH_LOG("    Normal map path = {0}", texturePath);
						textureHandle = AssetManager::CreateMemoryOnlyRendererAsset<Texture2D>(spec, texturePath);
					}
				...
				if (hasRoughnessMap)
				{
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						ANT_MESH_LOG("    Roughness map path = {0}", texturePath);
						textureHandle = AssetManager::CreateMemoryOnlyRendererAsset<Texture2D>(spec, texturePath);
					}
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						ANT_MESH_LOG("    Metalness map path = {0}", texturePath);
						mi->Set("u_MetalnessTexture", texture);
						mi->Set("u_MetalnessTexToggle", 1.0f);
					}
					else
					{
						ANT_CORE_ERROR_TAG("Mesh", "Could not load texture: {0}", texturePath);
					}
				}
				else
				{
					ANT_MESH_LOG("    No metalness texture");
				...
				for (uint32_t p = 0; p < aiMaterial->mNumProperties; p++)
				{
					...
					ANT_MESH_LOG("  Name = {0}", prop->mKey.data);
					// ANT_MESH_LOG("  Type = {0}", prop->mType);
					// ANT_MESH_LOG("  Size = {0}", prop->mDataLength);
					float data = *(float*)prop->mData;
					ANT_MESH_LOG("  Value = {0}", data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						ANT_MESH_LOG("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						ANT_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						ANT_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						ANT_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						ANT_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						ANT_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						ANT_MESH_LOG("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						ANT_MESH_LOG("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						ANT_MESH_LOG("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						ANT_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						ANT_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						ANT_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						ANT_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
#endif
					...
					if (prop->mType == aiPTI_String)
					{
						...
						if (key == "$raw.ReflectionFactor|file")
						{
							...
							{
								ANT_CORE_ERROR_TAG("Mesh", "    Could not load texture: {0}", str);
							}
							break;
						}
					}
		...
		{
			auto mi = Material::Create(Renderer::GetShaderLibrary()->Get("AntPBR_Static"), "Ant-Default");
