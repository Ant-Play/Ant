...
#include "Ant/Asset/AssetExtensions.h"
...
namespace Ant{
	...
	bool EditorAssetManager::ReloadData(AssetHandle assetHandle)
	{
		...
		if (!metadata.IsValid())
		{
			ANT_CORE_ERROR("Trying to reload invalid asset");
			return false;
		}
	...
	{
		ANT_CORE_INFO("[AssetManager] Loading Asset Registry");
		...
		if (!handles)
		{
			ANT_CORE_ERROR("[AssetManager] Asset Registry appears to be corrupted!");
		...
		for (auto entry : handles)
		{
			...
			{
				ANT_CORE_WARN_TAG("AssetManager", "Mismatch between stored AssetType and extension type when reading asset registry!");
				metadata.Type = GetAssetTypeFromPath(filepath);
			}

			if (!FileSystem::Exists(GetFileSystemPath(metadata)))
			{
				ANT_CORE_WARN("[AssetManager] Missing asset '{0}' detected in registry file, trying to locate...", metadata.FilePath);
				...
				for (auto& pathEntry : std::filesystem::recursive_directory_iterator(Project::GetAssetDirectory()))
				{
					...
					if (bestScore > 0)
						ANT_CORE_WARN("[AssetManager] Multiple candidates found...");
					...
					ANT_CORE_WARN("'{0}' has a score of {1}, best score is {2}", path.string(), score, bestScore);
				...
				{
					ANT_CORE_ERROR("[AssetManager] Failed to locate a potential match for '{0}'", metadata.FilePath);
					continue;
				}
				...
				ANT_CORE_WARN("[AssetManager] Found most likely match '{0}'", metadata.FilePath);
			}

			if (metadata.Handle == 0)
			{
				ANT_CORE_WARN("[AssetManager] AssetHandle for {0} is 0, this shouldn't happen.", metadata.FilePath);
				continue;
			}
		...
		ANT_CORE_INFO("[AssetManager] Loaded {0} asset entries", m_AssetRegistry.Count());
	}
	...
	void EditorAssetManager::WriteRegistryToFile()
	{
		...
		ANT_CORE_INFO("[AssetManager] serializing asset registry with {0} entries", sortedMap.size());
	...
	void EditorAssetManager::OnFileSystemChanged(const std::vector<FileSystemChangedEvent>& events)
	{
		...
		for (const auto& e : events)
		{
			if (!e.IsDirectory)
			{
				...
				{
					// TODO: This doesn't seem to work properly with MeshAssets.
					...
					// FIXME: This prevents the engine from crashing due to prefabs being modified on disk. This isn't the correct fix, since the issue is that we try to call mono functions from the watcher thread
					//		  but this does work for now. This fix will 100% cause issues at some point in the future, but until does the multi-threading pass of the engine, this will have to do.
