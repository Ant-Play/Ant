...
namespace Ant{
	...
	{
		if (extension == "asc") return AssetType::Scene;
		...
		if (extension == "apm") return AssetType::PhysicsMat;
	...
	{
		if (!FileSystem::Exists("assets/cache/AssetRegistryCache.atr"))
			return;

		std::ifstream stream("assets/cache/AssetRegistryCache.atr");
		...
		if (!handles)
		{
			ANT_CORE_ERROR("Failed to read Asset Registry file.");
			return;
		}
		...
		for (auto entry : handles)
		{
			...
			{
				ANT_CORE_WARN("Tried to load metadata for non-existing asset: {0}", metadata.FilePath);
				continue;
			}

			if (metadata.Handle == 0)
			{
				ANT_CORE_WARN("AssetHandle for {0} is 0, this shouldn't happen.", metadata.FilePath);
				continue;
			}
	...
	Ref<Asset> AssetManager::CreateAsset(const std::string& filepath, AssetType type, AssetHandle parentHandle)
	{
		...
		if (s_AssetRegistry.find(asset->FilePath) != s_AssetRegistry.end())
		{
			...
			if (asset->Type != type)
			{
				ANT_CORE_WARN("AssetType for '{0}' was different than the metadata. Did the file type change?", asset->FilePath);
	...
	void AssetManager::UpdateRegistryCache()
	{
		...
		std::ofstream fout("assets/cache/AssetRegistryCache.atr");
		fout << out.c_str();
	}
