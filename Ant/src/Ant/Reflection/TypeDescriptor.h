...
struct Ant::Type::Description<Class, Tag> : Ant::Type::MemberList<__VA_ARGS__>, Ant::Type::DescriptionInterface<Ant::Type::Description<Class,Tag>, Class, Tag, Ant::Type::MemberList<__VA_ARGS__>>	\
{																		\
private:																\
...
public:																	\
	...
	static constexpr std::array<std::string_view, NumberOfMembers> MemberNames										\
		= Ant::Utils::RemoveNamespace<NumberOfMembers>(																	\
			Ant::Utils::SplitString<NumberOfMembers>(MemberStr, Delimiter));												\
};
#endif // !DESCRIBED


#ifndef DESCRIBED
#define DESCRIBED(Class, ...) DESCRIBED_TAGGED(Class, Ant::Type::TDummyTag, __VA_ARGS__)
#endif
	...
	struct DescriptionInterface
	{
		...
		static constexpr std::optional<TValue> GetMemberValueByName(std::string_view memberName, const TObjType& object)
		{
			...
			return (NumberOfMembers > index) ? TList::GetMemberValueOfType<TValue>(index, object) : std::nullopt;
		}
		...
		{
			const auto index = IndexOf(memberName);
			return (NumberOfMembers > index) ? TList::GetMemberValue<TVariantType>(index, object) : TVariantType{};
		}
		...
		{
			const auto index = IndexOf(memberName);
			return (NumberOfMembers > index) ? TList::SetMemberValue<TValue>(index, value, object) : false;
		}
