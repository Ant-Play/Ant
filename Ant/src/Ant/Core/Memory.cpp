...
namespace Ant{
	...
	static Ant::AllocationStats s_GlobalStats;
...
#ifndef ANT_DIST
			if (!found)
				ANT_CORE_WARN_TAG("Memory", "Memory block {0} not present in alloc map", memory);
#endif
		}
		free(memory);
	}
...
{
	return Ant::Allocator::Allocate(size);
}
...
{
	return Ant::Allocator::Allocate(size, desc);
}
...
{
	return Ant::Allocator::Allocate(size, file, line);
}
...
{
	return Ant::Allocator::Free(memory);
}
