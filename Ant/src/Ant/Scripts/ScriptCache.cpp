...
namespace Ant {
	...
	{
		ANT_CORE_ASSERT(!s_Cache, "Trying to initialize ScriptCache multiple times!");
		s_Cache = anew Cache();
	...
	void ScriptCache::CacheClass(std::string_view className, MonoClass* monoClass)
	{
		...
		if (managedClass.FullName.find("Ant.") != std::string::npos)
		{
...
#define CACHE_ANT_CORE_CLASS(name) CacheClass("Ant." ##name, mono_class_from_name(ScriptEngine::GetCoreAssemblyInfo()->AssemblyImage, "Ant", name))
		...
		CACHE_ANT_CORE_CLASS("AssetHandle");
		CACHE_ANT_CORE_CLASS("Vector2");
		CACHE_ANT_CORE_CLASS("Vector3");
		CACHE_ANT_CORE_CLASS("Vector4");
		CACHE_ANT_CORE_CLASS("Entity");
		CACHE_ANT_CORE_CLASS("IEditorRunnable");
		CACHE_ANT_CORE_CLASS("Prefab");
		CACHE_ANT_CORE_CLASS("Mesh");
		CACHE_ANT_CORE_CLASS("StaticMesh");
		CACHE_ANT_CORE_CLASS("Material");
		CACHE_ANT_CORE_CLASS("Collider");
		CACHE_ANT_CORE_CLASS("BoxCollider");
		CACHE_ANT_CORE_CLASS("SphereCollider");
		CACHE_ANT_CORE_CLASS("CapsuleCollider");
		CACHE_ANT_CORE_CLASS("MeshCollider");
		CACHE_ANT_CORE_CLASS("PhysicsMaterial");
		CACHE_ANT_CORE_CLASS("RaycastHit2D");
		CACHE_ANT_CORE_CLASS("Texture2D");
	}
	...
	static void BuildClassMetadata(Ref<AssemblyInfo>& assemblyInfo, MonoClass* monoClass)
	{
		...
		if (parentClass != nullptr && parentClass != ANT_CACHED_CLASS_RAW("System.Object"))
		{
	...
	void ScriptCache::GenerateCacheForAssembly(Ref<AssemblyInfo> assemblyInfo)
	{
		...
		for (auto classID : assemblyInfo->Classes)
		{
			...
			if (mono_class_is_subclass_of(managedClass.Class, ANT_CACHED_CLASS_RAW("Ant.Entity"), false))
			{
	...
	MonoClass* ScriptCache::GetFieldTypeClass(FieldType fieldType)
	{
		switch (fieldType)
		{
		case FieldType::Bool: return ANT_CACHED_CLASS("System.Bool")->Class;
		case FieldType::Int8: return ANT_CACHED_CLASS("System.SByte")->Class;
		case FieldType::Int16: return ANT_CACHED_CLASS("System.Int16")->Class;
		case FieldType::Int32: return ANT_CACHED_CLASS("System.Int32")->Class;
		case FieldType::Int64: return ANT_CACHED_CLASS("System.Int64")->Class;
		case FieldType::UInt8: return ANT_CACHED_CLASS("System.Byte")->Class;
		case FieldType::UInt16: return ANT_CACHED_CLASS("System.UInt16")->Class;
		case FieldType::UInt32: return ANT_CACHED_CLASS("System.UInt32")->Class;
		case FieldType::UInt64: return ANT_CACHED_CLASS("System.UInt64")->Class;
		case FieldType::Float: return ANT_CACHED_CLASS("System.Single")->Class;
		case FieldType::Double: return ANT_CACHED_CLASS("System.Double")->Class;
		case FieldType::String: return ANT_CACHED_CLASS("System.String")->Class;
		case FieldType::Vector2: return ANT_CACHED_CLASS("Ant.Vector2")->Class;
		case FieldType::Vector3: return ANT_CACHED_CLASS("Ant.Vector3")->Class;
		case FieldType::Vector4: return ANT_CACHED_CLASS("Ant.Vector4")->Class;
		case FieldType::AssetHandle: return ANT_CACHED_CLASS("Ant.AssetHandle")->Class;
		case FieldType::Prefab: return ANT_CACHED_CLASS("Ant.Prefab")->Class;
		case FieldType::Entity: return ANT_CACHED_CLASS("Ant.Entity")->Class;
		case FieldType::Mesh: return ANT_CACHED_CLASS("Ant.Mesh")->Class;
		case FieldType::StaticMesh: return ANT_CACHED_CLASS("Ant.StaticMesh")->Class;
		case FieldType::Material: return ANT_CACHED_CLASS("Ant.Material")->Class;
		case FieldType::PhysicsMaterial: return ANT_CACHED_CLASS("Ant.PhysicsMaterial")->Class;
		case FieldType::Texture2D: return ANT_CACHED_CLASS("Ant.Texture2D")->Class;
		}
	...
	ManagedMethod* ScriptCache::GetManagedMethod(ManagedClass* managedClass, const std::string& name, bool ignoreParent /*= false*/)
	{
		...
		{
			ANT_CORE_ERROR_TAG("ScriptEngine", "Attempting to get method {0} from a nullptr class!", name);
			return nullptr;
		}
		...
		ANT_CORE_WARN_TAG("ScriptEngine", "Failed to find method with name: {0} in class {1}", name, managedClass->FullName);
		return nullptr;
	}
	...
	ManagedMethod* ScriptCache::GetSpecificManagedMethod(ManagedClass* managedClass, const std::string& name, uint32_t parameterCount, bool ignoreParent)
	{
		...
		if (method == nullptr)
			ANT_CORE_WARN_TAG("ScriptEngine", "Failed to find method with name: {0} and parameter count: {1} in class {2}", name, parameterCount, managedClass->FullName);
	...
	void ScriptCache::CacheClassFields(Ref<AssemblyInfo> assemblyInfo, ManagedClass& managedClass)
	{
		...
		while (currentClass != nullptr)
		{
			...
			if (classNameSpace.find("Ant") != std::string::npos && className.find("Entity") != std::string::npos)
			{
			...
			while ((field = mono_class_get_fields(currentClass, &fieldPtr)) != NULL)
			{
				...
				if (attributes && mono_custom_attrs_has_attr(attributes, GetManagedClassByName("Ant.ShowInEditorAttribute")->Class))
				{
					managedField.Flags &= ~(uint64_t)FieldFlag::Protected;
					managedField.Flags &= ~(uint64_t)FieldFlag::Internal;
					managedField.Flags &= ~(uint64_t)FieldFlag::Private;
					...
					MonoObject* attrib = mono_custom_attrs_get_attr(attributes, GetManagedClassByName("Ant.ShowInEditorAttribute")->Class);
	...
	void ScriptCache::CacheClassProperties(Ref<AssemblyInfo> assemblyInfo, ManagedClass& managedClass)
	{
		...
		while ((prop = mono_class_get_properties(managedClass.Class, &propertyIter)) != NULL)
		{
			...
			// NOTE: Parsing attributes for properties is not that straightforward.
			...
			if (monoType == nullptr)
			{
				ANT_CORE_ERROR_TAG("ScriptEngine", "Failed to retrieve managed type for property '{0}'", name);
				continue;
			}

			MonoCustomAttrInfo* attributes = mono_custom_attrs_from_property(managedClass.Class, prop);

			if (!TypeUtils::ContainsAttribute(attributes, "Ant.ShowInEditorAttribute"))
				continue;
