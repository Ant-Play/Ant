...
namespace Ant{
	...
	void GCManager::Init()
	{
		ANT_CORE_ASSERT(!s_GCState, "Trying to initialize GC Manager multiple times!");
	...
	void GCManager::Shutdown()
	{
		...
		{
			ANT_CORE_ERROR("ScriptEngine", "Memory leak detected!");
			ANT_CORE_ERROR("ScriptEngine", "Not all GCHandles have been cleaned up!");
	...
	{
		ANT_PROFILE_FUNC();
		ANT_CORE_INFO_TAG("ScriptEngine", "Collecting garbage...");
		mono_gc_collect(mono_gc_max_generation());
		if (blockUntilFinalized)
		{
			while (mono_gc_pending_finalizers());
			ANT_CORE_INFO_TAG("ScriptEngine", "GC Finished...");
		}
	}
	...
	GCHandle GCManager::CreateObjectReference(MonoObject* managedObject, bool weakReference, bool pinned, bool track)
	{
		...
		ANT_CORE_ASSERT(handle, "Failed to retrieve valid GC Handle!");
	...
	void GCManager::ReleaseObjectReference(GCHandle handle)
	{
		...
		{
			ANT_CORE_ERROR_TAG("ScriptEngine", "Tried to release an object reference using an invalid handle!");
			return;
		}
