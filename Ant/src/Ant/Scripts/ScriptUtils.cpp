...
#define REGISTER_METHOD_THUNK(var, method) s_MethodThunks->var.SetThunkFromMethod(method);
		...
		REGISTER_METHOD_THUNK(Entity_OnCreate, ANT_CACHED_METHOD("Ant.Entity", "OnCreate", 0));
		REGISTER_METHOD_THUNK(Entity_OnUpdate, ANT_CACHED_METHOD("Ant.Entity", "OnUpdate", 1));
		REGISTER_METHOD_THUNK(Entity_OnPhysicsUpdate, ANT_CACHED_METHOD("Ant.Entity", "OnPhysicsUpdate", 1));
		REGISTER_METHOD_THUNK(Entity_OnDestroyInternal, ANT_CACHED_METHOD("Ant.Entity", "OnDestroyInternal", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnInstantiate, ANT_CACHED_METHOD("Ant.IEditorRunnable", "OnInstantiate", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnUIRender, ANT_CACHED_METHOD("Ant.IEditorRunnable", "OnUIRender", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnShutdown, ANT_CACHED_METHOD("Ant.IEditorRunnable", "OnShutdown", 0));
...
#undef REGISTER_METHOD_THUNK
	...
	bool ScriptUtils::CheckMonoError(MonoError& error)
	{
		...
		if (hasError)
		{
			...
			ANT_CORE_ERROR_TAG("ScriptEngine", "Mono Error!");
			ANT_CORE_ERROR_TAG("ScriptEngine", "\tError Code: {0}", errorCode);
			ANT_CORE_ERROR_TAG("ScriptEngine", "\tError Message: {0}", errorMessage);
	...
	void ScriptUtils::HandleException(MonoObject* exception)
	{
		...
		ANT_CONSOLE_LOG_ERROR("{0}: {1}. Source: {2}, Stack Trace: {3}", exceptionInfo.TypeName, exceptionInfo.Message, exceptionInfo.Source, exceptionInfo.StackTrace);
	}
	...
	MonoObject* ScriptUtils::ValueToMonoObject(const void* data, FieldType dataType)
	{
		...
		else
		{
			switch (dataType)
			{
				...
				case FieldType::Prefab: return ScriptEngine::CreateManagedObject("Ant.Prefab", *(AssetHandle*)data);
				case FieldType::Entity: return ScriptEngine::CreateManagedObject("Ant.Entity", *(UUID*)data);
				case FieldType::Mesh: return ScriptEngine::CreateManagedObject("Ant.Mesh", *(AssetHandle*)data);
				case FieldType::StaticMesh: return ScriptEngine::CreateManagedObject("Ant.StaticMesh", *(AssetHandle*)data);
				case FieldType::Material: return ScriptEngine::CreateManagedObject("Ant.Material", *(AssetHandle*)data);
				case FieldType::PhysicsMaterial: return ScriptEngine::CreateManagedObject("Ant.PhysicsMaterial", *(AssetHandle*)data);
				case FieldType::Texture2D: return ScriptEngine::CreateManagedObject("Ant.Texture2D", *(AssetHandle*)data);
			}
		}

		ANT_CORE_ASSERT(false, "Unsupported value type!");
		return nullptr;
	}
	...
	/*MonoObject* ScriptUtils::GetDefaultValueObjectForType(const ManagedType& type)
	{
		...
		switch (type.NativeType)
		{
			case UnmanagedType::Bool: return BoxValue(ANT_CACHED_CLASS("System.Boolean"), (MonoBoolean)false);
			case UnmanagedType::Int8: return BoxValue(ANT_CACHED_CLASS("System.SByte"), 0);
			case UnmanagedType::Int16: return BoxValue(ANT_CACHED_CLASS("System.Int16"), 0);
			case UnmanagedType::Int32: return BoxValue(ANT_CACHED_CLASS("System.Int32"), 0);
			case UnmanagedType::Int64: return BoxValue(ANT_CACHED_CLASS("System.Int64"), 0);
			case UnmanagedType::UInt8: return BoxValue(ANT_CACHED_CLASS("System.Byte"), 0);
			case UnmanagedType::UInt16: return BoxValue(ANT_CACHED_CLASS("System.UInt16"), 0);
			case UnmanagedType::UInt32: return BoxValue(ANT_CACHED_CLASS("System.UInt32"), 0);
			case UnmanagedType::UInt64: return BoxValue(ANT_CACHED_CLASS("System.UInt64"), 0);
			case UnmanagedType::Float: return BoxValue(ANT_CACHED_CLASS("System.Single"), 0);
			case UnmanagedType::Double: return BoxValue(ANT_CACHED_CLASS("System.Double"), 0);
			case UnmanagedType::String: return (MonoObject*)UTF8StringToMono("");
			case UnmanagedType::AssetHandle: return BoxValue(ANT_CACHED_CLASS("Ant.AssetHandle"), 0);
			case UnmanagedType::Vector2: return BoxValue(ANT_CACHED_CLASS("Ant.Vector2"), glm::vec2(0.0f));
			case UnmanagedType::Vector3: return BoxValue(ANT_CACHED_CLASS("Ant.Vector3"), glm::vec3(0.0f));
			case UnmanagedType::Vector4: return BoxValue(ANT_CACHED_CLASS("Ant.Vector4"), glm::vec4(0.0f));
	...
	FieldType ScriptUtils::GetFieldTypeFromMonoType(MonoType* monoType)
	{
		...
		switch (typeEncoding)
		{
			...
			{
				if (ANT_CORE_CLASS(AssetHandle) && typeClass == ANT_CORE_CLASS(AssetHandle)->Class)
					return FieldType::AssetHandle;

				if (ANT_CORE_CLASS(Vector2) && typeClass == ANT_CORE_CLASS(Vector2)->Class)
					return FieldType::Vector2;

				if (ANT_CORE_CLASS(Vector3) && typeClass == ANT_CORE_CLASS(Vector3)->Class)
					return FieldType::Vector3;

				if (ANT_CORE_CLASS(Vector4) && typeClass == ANT_CORE_CLASS(Vector4)->Class)
			...
			{
				auto entityClass = ANT_CORE_CLASS(Entity);

				if (entityClass && mono_class_is_assignable_from(typeClass, entityClass->Class))
					return FieldType::Entity;

				if (ANT_CORE_CLASS(Prefab) && typeClass == ANT_CORE_CLASS(Prefab)->Class)
					return FieldType::Prefab;

				if (ANT_CORE_CLASS(Mesh) && typeClass == ANT_CORE_CLASS(Mesh)->Class)
					return FieldType::Mesh;

				if (ANT_CORE_CLASS(StaticMesh) && typeClass == ANT_CORE_CLASS(StaticMesh)->Class)
					return FieldType::StaticMesh;

				if (ANT_CORE_CLASS(Material) && typeClass == ANT_CORE_CLASS(Material)->Class)
					return FieldType::Material;

				if (ANT_CORE_CLASS(PhysicsMaterial) && typeClass == ANT_CORE_CLASS(PhysicsMaterial)->Class)
					return FieldType::PhysicsMaterial;

				if (ANT_CORE_CLASS(Texture2D) && typeClass == ANT_CORE_CLASS(Texture2D)->Class)
	...
	//{
	//	ANT_CORE_VERIFY(arr, "Can't get a value from a nullptr array");

	//	uintptr_t length = mono_array_length(arr);
	//	ANT_CORE_VERIFY(index < length, "index out of range");
	...
	//{
	//	ANT_CORE_VERIFY(arr && *arr, "Can't resize a nullptr array");
	//	ANT_CORE_VERIFY(newLength > 0, "Can't resize an array to a length of 0");

	//	uintptr_t length = mono_array_length(*arr);
	//	ANT_CORE_VERIFY(length > 0, "Cannot resize an array of length 0");
	...
	//{
	//	ANT_CORE_VERIFY(arr && *arr, "Cannot remove elements from nullptr array");
	...
	{
		ANT_CORE_VERIFY(!arrayClass.empty(), "Cannot create managed array of no type");

		ManagedClass* klass = ScriptCache::GetManagedClassByName(arrayClass);
		ANT_CORE_VERIFY(klass, "Unable to find array class");
	...
	{
		ANT_CORE_VERIFY(arrayClass, "Cannot create managed array of no type");
	...
	void ManagedArrayUtils::SetValueInternal(MonoArray* arr, uintptr_t index, void* data)
	{
		...
		if (index >= length)
		{
			ANT_CORE_WARN_TAG("ScriptEngine", "Index out of bounds in C# array!");
			return;
		}
