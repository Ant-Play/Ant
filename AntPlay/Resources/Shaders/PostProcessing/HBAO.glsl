...
{
	return vec3(fma(uv, u_HBAO.PerspectiveInfo.xy, u_HBAO.PerspectiveInfo.zw) * (u_HBAO.IsOrtho ? 1.0 : eye_z), eye_z);
}
...
void main()
{
	...
	float radiusPixels = u_HBAO.RadiusToScreen / (u_HBAO.IsOrtho ? 1.0 : viewPosition.z);
