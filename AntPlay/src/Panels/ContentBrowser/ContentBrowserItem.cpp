...
// TEMP
#include "Ant/Asset/MeshRuntimeSerializer.h"
...
namespace Ant {
	...
	void ContentBrowserDirectory::OnRenamed(const std::string& newName)
	{
		...
		{
			ANT_CORE_ERROR("Couldn't rename {0} to {1}!", m_DirectoryInfo->FilePath.filename().string(), newName);
		}
	}
	...
	void ContentBrowserDirectory::Delete()
	{
		...
		if (!deleted)
		{
			ANT_CORE_ERROR("Failed to delete folder {0}", m_DirectoryInfo->FilePath);
			return;
		}
	...
	void ContentBrowserAsset::Delete()
	{
		...
		if (!deleted)
		{
			ANT_CORE_ERROR("Couldn't delete {0}", m_AssetInfo.FilePath);
			return;
		}
	...
	bool ContentBrowserAsset::Move(const std::filesystem::path& destination)
	{
		...
		if (!wasMoved)
		{
			ANT_CORE_ERROR("Couldn't move {0} to {1}", m_AssetInfo.FilePath, destination);
			return false;
		}
	...
	void ContentBrowserAsset::OnRenamed(const std::string& newName)
	{
		...
		{
			ANT_CORE_ERROR("Couldn't rename {0} to {1}!", filepath.filename().string(), newName);
		}
	}
