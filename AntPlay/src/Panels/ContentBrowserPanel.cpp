...
namespace Ant {
		...
		: m_Project(nullptr)
	{
		...
		m_AssetIconMap[".amaterial"] = EditorResources::MaterialFileIcon;
		m_AssetIconMap[".ascene"] = EditorResources::SceneFileIcon;
		m_AssetIconMap[".aprefab"] = EditorResources::PrefabFileIcon;
		m_AssetIconMap[".aanim"] = EditorResources::AnimationFileIcon;
		m_AssetIconMap[".aanimc"] = EditorResources::AnimationControllerFileIcon;
		m_AssetIconMap[".amesh"] = EditorResources::MeshFileIcon;
		m_AssetIconMap[".asmesh"] = EditorResources::StaticMeshFileIcon;
		m_AssetIconMap[".amc"] = EditorResources::MeshColliderFileIcon;
		m_AssetIconMap[".apm"] = EditorResources::PhysicsMaterialFileIcon;
		m_AssetIconMap[".askel"] = EditorResources::SkeletonFileIcon;
		m_AssetIconMap[".asoundc"] = EditorResources::SoundConfigFileIcon;
							...
							if (ImGui::BeginMenu("New"))
							{
								...
								if (ImGui::BeginMenu("Physics"))
								{
									if (ImGui::MenuItem("Physics Material"))
										CreateAsset<PhysicsMaterial>("New Physics Material.apm", 0.6f, 0.6f, 0.0f);

									if (ImGui::MenuItem("Mesh Collider"))
										CreateAsset<MeshColliderAsset>("New Mesh Collider.amc");

									ImGui::EndMenu();
								}

								if (ImGui::MenuItem("Animation Controller"))
									CreateAsset<AnimationController>("New Animation Controller.aanimc");

								if (ImGui::MenuItem("Material"))
									CreateAsset<MaterialAsset>("New Material.amaterial");

								if (ImGui::BeginMenu("Audio"))
								{
									if (ImGui::MenuItem("Sound Config"))
										CreateAsset<SoundConfig>("New Sound Config.asoundc");
				...
				if (ImGui::BeginDragDropTarget())
				{
					...
					if (data)
					{
						...
						for (size_t i = 0; i < count; i++)
						{
							...
							if (!entity)
							{
								ANT_CORE_ERROR_TAG("ContentBrowser", "Failed to find Entity with ID {0} in current Scene context!", entityID);
								continue;
							}

							Ref<Prefab> prefab = CreateAsset<Prefab>(entity.Name() + ".aprefab");
	...
	void ContentBrowserPanel::OnProjectChanged(const Ref<Project>& project)
	{
		...
		Project::GetEditorAssetManager()->SetAssetChangeCallback(ANT_BIND_EVENT_FN(ContentBrowserPanel::OnFileSystemChanged));
	...
	void ContentBrowserPanel::PasteCopiedAssets()
	{
		...
		for (AssetHandle copiedAsset : m_CopiedAssets)
		{
			...
			if (item->GetType() == ContentBrowserItem::ItemType::Asset)
			{
				...
				ANT_CORE_VERIFY(!std::filesystem::exists(filepath));
	...
	void ContentBrowserPanel::RenderDirectoryHierarchy(Ref<DirectoryInfo>& directory)
	{
		...
		if (ImGui::BeginPopupContextItem())
		{
			if (ImGui::BeginMenu("New"))
			{
				...
				if (ImGui::MenuItem("Scene"))
					CreateAssetInDirectory<Scene>("New Scene.ascene", directory);

				if (ImGui::MenuItem("Material"))
					CreateAssetInDirectory<MaterialAsset>("New Material.amaterial", directory);

				if (ImGui::MenuItem("Physics Material"))
					CreateAssetInDirectory<PhysicsMaterial>("New Physics Material.apm", directory, 0.6f, 0.6f, 0.0f);

				if (ImGui::MenuItem("Sound Config"))
					CreateAssetInDirectory<SoundConfig>("New Sound Config.asoundc", directory);
	...
	void ContentBrowserPanel::RenderItems()
	{
		...
		// TODO: This method of handling actions isn't great... It's starting to become spaghetti...
		...
		for (auto& item : m_CurrentItems)
		{
			...
			if (result.IsSet(ContentBrowserAction::Renamed))
			{
				...
				// NOTE : Calling the next line of code will cause the folder before the renaming to be selected as well, 
