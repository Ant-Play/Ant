...
namespace Ant {
	...
	{
		m_Pages.push_back({ "AntPlay", [this]() { DrawAntPlayPage(); } });
	...
	void ApplicationSettingsPanel::OnImGuiRender(bool& isOpen)
	{
		...
		if (ImGui::Begin("Application Settings", &isOpen))
		{
				...
				ImGui::GetContentRegionAvail().y - 8.0f)))
			{
				...
				{
					ANT_CORE_ERROR_TAG("ApplicationSettingsPanel", "Invalid page selected!");
				}
	...
	void ApplicationSettingsPanel::DrawScriptingPage()
	{
		...
		UI::BeginPropertyGrid();
		{
			saveSettings |= UI::Property("Show Hidden Fields", editorSettings.ShowHiddenFields);
			UI::SetTooltip("Determines if Script Components will display \"hidden\" fields, e.g fields that have been deleted, or are non-public.");

			saveSettings |= UI::Property("Debugger Listening Port", editorSettings.ScriptDebuggerListenPort, 1024, 49151); // Anything between 1024 and 49151 should be User ports and should be safe (could still clash)
			UI::SetTooltip("The port that the Mono Debugger will listen on. Requires restart if changed. Only used with the Ant Tools extension for Visual Studio. (Default: 2550)");
		}
		UI::EndPropertyGrid();
	...
	void ApplicationSettingsPanel::DrawAntPlayPage()
	{
		auto& editorSettings = ApplicationSettings::Get();
		bool saveSettings = false;
		...
		UI::BeginPropertyGrid();
		{
			...
			UI::SetTooltip("Shows hidden options in AntPlay which can be used to fix things.");
		}
		UI::EndPropertyGrid();
