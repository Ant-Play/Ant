	...
	void SceneRendererPanel::OnImGuiRender(bool& isOpen)
	{
		...
		if (ImGui::Begin("Scene Renderer", &isOpen) && m_Context)
		{
			...
			if (UI::PropertyGridHeader("Render Statistics"))
			{
				...
				ImGui::Text("FPS: %.3fms", commandBuffer->GetExecutionGPUTime(frameIndex) / 1000.0f);
			...
			if (UI::PropertyGridHeader("Ambient Occlusion"))
			{
				...
				if (UI::BeginTreeNode("Ground-Truth Ambient Occlusion"))
				{
					...
					if (UI::Property("Enable", options.EnableGTAO))
					{
						...
						Renderer::SetGlobalMacroInShaders("__ANT_AO_METHOD", fmt::format("{}", ShaderDef::GetAOMethod(options.EnableGTAO, options.EnableHBAO)));
						Renderer::SetGlobalMacroInShaders("__ANT_REFLECTION_OCCLUSION_METHOD", std::to_string((int)options.ReflectionOcclusionMethod));
					}
					if (UI::Property("Bent Normals", options.GTAOBentNormals))
					{
						Renderer::SetGlobalMacroInShaders("__ANT_GTAO_COMPUTE_BENT_NORMALS", fmt::format("{}", (int)options.GTAOBentNormals));
			...
			if (UI::PropertyGridHeader("SSR"))
			{
				...
				if (UI::PropertyDropdown("Reflection Occlusion method", aoMethods, 4, methodIndex))
				{
					...
					Renderer::SetGlobalMacroInShaders("__ANT_REFLECTION_OCCLUSION_METHOD", fmt::format("{}", options.ReflectionOcclusionMethod));
					Renderer::SetGlobalMacroInShaders("__ANT_AO_METHOD", fmt::format("{}", ShaderDef::GetAOMethod(options.EnableGTAO, options.EnableHBAO)));
				}
