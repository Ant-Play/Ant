...
#include "Ant/Renderer/ShaderPack.h"
#include "Ant/Scripts/ScriptEngine.h"
#include "Ant/Scripts/ScriptBuilder.h"
#include "Ant/Editor/AssetEditorPanel.h"
#include "Ant/Editor/SelectionManager.h"
#include "Ant/Audio/Editor/AudioEventsEditor.h"
#include "Ant/Editor/ScriptEngineDebugPanel.h"
...
#include "Ant/Physics/3D/Debug/PhysXDebugger.h"
...
#define SCENE_RENDERER_PANEL_ID "SceneRendererPanel"
	...
	void EditorLayer::OnAttach()
	{
		...
		// Should we ever want to actually show editor layer panels in Ant::Runtime
		...
		if (!m_UserPreferences->StartupProject.empty())
			...
			ANT_CORE_VERIFY(false, "No project provided!");
...
#if 0
		aAssetPack assetPack(Project::GetAssetDirectory() / "AssetPack.aap");
	...
	{
		ShaderPack::CreateFromLibrary(Renderer::GetShaderLibrary(), "Resources/ShaderPack.asp");
		UI::ShowSimpleMessageBox<ANT_MESSAGE_BOX_OK_BUTTON>(
			"Created ShaderPack!",
			"Successfully created shader pack. File Path: {0}", std::filesystem::absolute("Resources/ShaderPack.asp").string()
		);
	}
	...
	void EditorLayer::BuildAssetPack()
	{
		...
		ANT_CONSOLE_LOG_INFO("Building Asset Pack...");
	...
	{
		ANT_CORE_VERIFY(RendererAPI::Current() == RendererAPIType::Vulkan, "Only Vulkan is supported!");
		const std::string rendererAPI = "Vulkan";
		const std::string title = fmt::format("{0} ({1}) - AntPlay - {2} ({3}) Renderer: {4}", sceneName, Project::GetActive()->GetConfig().Name, Application::GetPlatformName(), Application::GetConfigurationName(), rendererAPI);
				...
				if (ImGui::BeginMenu("File"))
				{
					...
					if (ImGui::MenuItem("Build Sound Bank"))
					{
						if (!MiniAudioEngine::BuildSoundBank())
							UI::ShowSimpleMessageBox<ANT_MESSAGE_BOX_OK_BUTTON>(":[", "Failed to build Sound Bank.");
					}
	...
	float EditorLayer::UI_DrawTitlebar()
	{
		...
		// Logo
		{
			const int logoWidth = EditorResources::AntLogoTexture->GetWidth();
			const int logoHeight = EditorResources::AntLogoTexture->GetHeight();
			const ImVec2 logoOffset(16.0f + windowPadding.x, 8.0f + windowPadding.y);
			const ImVec2 logoRectStart = { ImGui::GetItemRectMin().x + logoOffset.x, ImGui::GetItemRectMin().y + logoOffset.y };
			const ImVec2 logoRectMax = { logoRectStart.x + logoWidth, logoRectStart.y + logoHeight };
			drawList->AddImage(UI::GetTextureID(EditorResources::AntLogoTexture), logoRectStart, logoRectMax);
		}
	...
	void EditorLayer::UI_ViewportSettings()
	{
		...
		// Draw the settings popup
		{
			...
			{
				// NOTE: This method is currently only called for the main viewport because the second viewport is currently a bit broken
	...
	void EditorLayer::UI_DrawGizmos()
	{
		...
		bool snap = Input::IsKeyDown(ANT_KEY_LEFT_CONTROL);
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		if (((Input::IsKeyDown(Ant::KeyCode::LeftAlt) && (Input::IsMouseButtonDown(MouseButton::Left) || (Input::IsMouseButtonDown(MouseButton::Middle)))) || Input::IsMouseButtonDown(MouseButton::Right)) && !m_StartedCameraClickInViewport && m_ViewportPanelFocused && m_ViewportPanelMouseOver)
			m_StartedCameraClickInViewport = true;

		if (!Input::IsMouseButtonDown(MouseButton::Right) && !(Input::IsKeyDown(Ant::KeyCode::LeftAlt) && (Input::IsMouseButtonDown(MouseButton::Left) || (Input::IsMouseButtonDown(MouseButton::Middle)))))
	...
	void EditorLayer::CreateProject(std::filesystem::path projectPath)
	{
		...
		std::filesystem::path antRootDirectory = std::filesystem::absolute("./Resources").parent_path().string();
		std::string antRootDirectoryString = antRootDirectory.string();

		if (antRootDirectory.stem().string() == "AntPlay")
			antRootDirectoryString = antRootDirectory.parent_path().string();

		std::replace(antRootDirectoryString.begin(), antRootDirectoryString.end(), '\\', '/');
		...
		{
			// Project File
			std::ifstream stream(projectPath / "Project.aproj");
			ANT_CORE_VERIFY(stream.is_open());
			std::stringstream ss;
			ss << stream.rdbuf();
			stream.close();
			...
			std::ofstream ostream(projectPath / "Project.aproj");
			ostream << str;
			ostream.close();

			std::string newProjectFileName = std::string(s_ProjectNameBuffer) + ".aproj";
			std::filesystem::rename(projectPath / "Project.aproj", projectPath / newProjectFileName);
		}
		...
		{
			std::filesystem::path originalFilePath = antRootDirectoryString + "/AntPlay/Resources/Meshes/Default";
			std::filesystem::path targetPath = projectPath / "Assets" / "Meshes" / "Default" / "Source";
			ANT_CORE_VERIFY(std::filesystem::exists(originalFilePath));
...

		{
			...
			projectEntry.FilePath = (projectPath / (std::string(s_ProjectNameBuffer) + ".aproj")).string();
			projectEntry.LastOpened = time(NULL);
			m_UserPreferences->RecentProjects[projectEntry.LastOpened] = projectEntry;
		...
		OpenProject(projectPath.string() + "/" + std::string(s_ProjectNameBuffer) + ".aproj");
	}
	...
	{
		std::filesystem::path filepath = FileSystem::OpenFileDialog("Ant Project (*.aproj)\0*.aproj\0");
	...
	void EditorLayer::OpenProject(const std::filesystem::path& filepath)
	{
		...
		{
			ANT_CORE_ERROR("Tried to open a project that doesn't exist. Project path: {0}", filepath);
			memset(s_OpenProjectFilePathBuffer, 0, MAX_PROJECT_FILEPATH_LENGTH);
			return;
		}
		...
		{
			ANT_CONSOLE_LOG_WARN("No C# assembly has been provided in the Project Settings, or it wasn't found. Please make sure to build the Visual Studio Solution for this project if you want to use C# scripts!");
			std::string path = appAssemblyPath.string();
			if (path.empty())
				path = "<empty>";
			ANT_CONSOLE_LOG_WARN("App Assembly Path = {}", path);
		}
	...
	void EditorLayer::CloseProject(bool unloadProject)
	{
		...
		ANT_CORE_ASSERT(m_EditorScene->GetRefCount() == 1, "Scene will not be destroyed after project is closed - something is still holding scene refs!");
	...
	bool EditorLayer::OpenScene(const std::filesystem::path& filepath, const bool checkAutoSave)
	{
		if (!FileSystem::Exists(filepath))
		{
			ANT_CORE_ERROR("Tried loading a non-existing scene: {0}", filepath);
			return false;
		}
		...
		// NOTE: We set the scene context to nullptr here to make sure all old script entities have been destroyed
		ScriptEngine::SetSceneContext(nullptr, nullptr);
	...
	void EditorLayer::SaveSceneAuto()
	{
		if (!m_SceneFilePath.empty())
		{
			...
			// TODO:  ACR shouldn't be coupled with the scene, it's scope is Project.
	...
	void EditorLayer::UI_ShowWelcomePopup()
	{
		UI::ShowMessageBox("Welcome", [this]()
			{
				ImGui::Text("Welcome to Ant!");
				ImGui::Separator();
				ImGui::TextWrapped("Ant is an early-stage interactive application and rendering engine for Windows.");
				ImGui::TextWrapped("Please report bugs by opening an issue on the Ant-dev GitLab.");
	...
	{
		UI::ShowMessageBox("About Ant", []()
			{
				UI::Fonts::PushFont("Large");
				ImGui::Text("Ant Engine");
				UI::Fonts::PopFont();
				...
				UI::Fonts::PushFont("Bold");
				ImGui::Text("Ant Core Team");
				UI::Fonts::PopFont();
				ImGui::Text("HJC");
				ImGui::Separator();

				ImGui::TextUnformatted("For more info, visit: xxx.com");
	...
	void EditorLayer::UI_BuildAssetPackDialog()
	{
		if (m_AssetPackFuture.valid())
		{
			...
			if (status == std::future_status::ready)
			{
				...
				m_AssetPackFuture = {};
				ANT_CONSOLE_LOG_INFO("Asset Pack build done!");
			}
		}
		...
		UI::ShowMessageBox("Create New Mesh", [this, meshAssetPathLabel, skeletonAssetPathLabel, animationAssetPathLabel]()
			{
				...
				std::string filepathMesh = fmt::format("{0}/{1}.{2}", m_CurrentScene->GetName(), assetData.FilePath.stem().string(), meshType == MeshType::Static ? "asmesh" : "amesh");
				std::string filepathSkeleton = fmt::format("{0}/{1}.{2}", m_CurrentScene->GetName(), assetData.FilePath.stem().string(), "askel");
				std::string filepathAnimation = fmt::format("{0}/{1}.{2}", m_CurrentScene->GetName(), assetData.FilePath.stem().string(), "aanim");
				...
				ANT_CORE_ASSERT(m_CreateNewMeshPopupData.MeshToCreate);
				...
				if (ImGui::Button("Create"))
				{
					...
					{
						if (m_CreateNewMeshPopupData.MeshToCreate->GetAnimationCount() > 0)
						{
							std::string serializePath = m_CreateNewMeshPopupData.CreateAnimationFilenameBuffer.data();
							std::filesystem::path path = Project::GetActive()->GetAnimationPath() / serializePath;
							if (!FileSystem::Exists(path.parent_path()))
								FileSystem::CreateDirectory(path.parent_path());

							animationAsset = Project::GetEditorAssetManager()->CreateNewAsset<AnimationAsset>(path.filename().string(), path.parent_path().string(), m_CreateNewMeshPopupData.MeshToCreate);
						}
						else
						{
							UI_ShowNoAnimationPopup();
						}
					}

					if (doCreateAnimationController)
					{
						std::string serializePath = m_CreateNewMeshPopupData.CreateAnimationFilenameBuffer.data();
						std::filesystem::path path = Project::GetActive()->GetAnimationPath() / serializePath;
						path.replace_extension(".aanimc");
						if (!FileSystem::Exists(path.parent_path()))
							FileSystem::CreateDirectory(path.parent_path());

						auto animationController = Project::GetEditorAssetManager()->CreateNewAsset<AnimationController>(path.filename().string(), path.parent_path().string());
						animationController->SetSkeletonAsset(skeletonAsset);
						if (animationAsset)
						{
							for (uint32_t i = 0; i < animationAsset->GetAnimationCount(); ++i)
							{
								auto state = Ref<AnimationState>::Create(animationAsset, i);
								state->SetValid(
									animationAsset &&
									...
									animationAsset->GetMeshSource()->IsCompatibleSkeleton(i, skeletonAsset->GetSkeleton())
								);
								animationController->AddState("", state);
							}
						}
						AssetImporter::Serialize(animationController);
						...
						if (entity)
						{
							...
							anim.AnimationController = animationController->Handle;
							...
							anim.AnimationData->m_StateIndex = 0;
							anim.BoneEntityIds = m_CurrentScene->FindBoneEntityIds(entity, entity, animationController);
						}
					}
...
#else
					ImGui::TextColored(ImVec4(0.9f, 0.35f, 0.3f, 1.0f), "Memory is not being tracked because ANT_TRACK_MEMORY is not defined!");
#endif
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyDown(ANT_KEY_LEFT_CONTROL) && !Input::IsMouseButtonDown(MouseButton::Right))
		{
			switch (e.GetKeyCode())
			{
				case KeyCode::B:
			...
			if (Input::IsKeyDown(ANT_KEY_LEFT_SHIFT))
			{
				switch (e.GetKeyCode())
				{
					case KeyCode::S:
		...
		if (m_SceneState == SceneState::Play && Input::IsKeyDown(ANT_KEY_LEFT_ALT))
		{
