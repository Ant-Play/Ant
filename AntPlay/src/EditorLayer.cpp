...
namespace Ant {
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		{
			Ant::Renderer::BeginRenderPass(Ant::SceneRenderer::GetFinalRenderPass(), false);
			auto viewProj = m_CameraEntity.GetComponent<CameraComponent>().Camera.GetViewProjection();
			Ant::Renderer2D::BeginScene(viewProj, false);
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		case ANT_KEY_G:
			// Toggle grid
			if (Ant::Input::IsKeyPressed(ANT_KEY_LEFT_CONTROL))
	...
	bool EditorLayer::OnMouseButtonPressed(MouseButtonPressedEvent& e)
	{
		...
		if (e.GetMouseButton() == ANT_MOUSE_BUTTON_LEFT && !Input::IsKeyPressed(ANT_KEY_LEFT_ALT) && !ImGuizmo::IsOver())
		{
