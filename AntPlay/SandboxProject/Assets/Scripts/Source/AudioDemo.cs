...
namespace AudioDemo
{
	...
	class SoundCone : Entity
	{
		...
		protected override void OnUpdate(float ts)
		{
...

			{
				...
				if(sine == 1.0f)
				{
					float rv = Ant.Random.Float() * 0.2f;
					float rp = Ant.Random.Float() * 0.4f;
	...
	class TriggerVolume : Entity 
	{
		...
		void PlaySound()
		{
			float x = Ant.Random.Float() * xBounds - 0.5f * xBounds + Transform.Translation.X;
			float z = Ant.Random.Float() * zBounds - 0.5f * zBounds + Transform.Translation.Z;
	...
	class OneShotTrigger : Entity
	{
		...
		{
			float rv = Ant.Random.Float() * 0.2f;
			float rp = Ant.Random.Float() * 0.06f;
