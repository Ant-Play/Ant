...
#define MAX_PROJECT_FILEPATH_LENGTH 512
	...
	{
		// TODO: Use our own png
		std::string path = "Resources/Editor/hazel.png";
		m_AntLogoTexture = Texture2D::Create(TextureSpecification(), path);
	}
	...
	void LauncherLayer::OnImGuiRender()
	{
		...
		// Ant Install Folder Prompt
		{
			if ((m_Properties.InstallPath.empty() || !FileSystem::Exists(m_Properties.InstallPath)) && !ImGui::IsPopupOpen("Select Ant Install"))
			{
				ImGui::OpenPopup("Select Ant Install");
			...
			if (ImGui::BeginPopupModal("Select Ant Install", 0, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize))
			{
				ImGui::PushFont(boldFont);
				ImGui::TextUnformatted("Failed to find an appropiate Ant installation!");
				ImGui::PopFont();

				ImGui::TextWrapped("Please select the root folder for the Ant version you want to use (E.g C:/Dev/Ant-dev).You should be able to find a file called premake5.lua in the root folder. The install you select will be used when creating new projects.");
				...
				ImGui::InputTextWithHint("##ant_install_location", "C:/Dev/Ant-dev/", m_Properties.InstallPath.data(), MAX_PROJECT_FILEPATH_LENGTH, ImGuiInputTextFlags_ReadOnly);
				ImGui::SameLine();
				if (ImGui::Button("..."))
				{
				...
				{
					bool success = FileSystem::SetEnvironmentVariable("ANT_DIR", m_Properties.InstallPath);
					ANT_CORE_ASSERT(success, "Failed to set Environment Variable!");
		...
		ImGui::BeginChild("info_area");
		{
			...
			UI::Image(m_AntLogoTexture, ImVec2(imageSize, imageSize));
		...
		ImGui::BeginChild("general_area");
		{
			...
			{
				std::string result = FileSystem::OpenFileDialog("Ant Project (*.aproj)\0*.aproj\0").string();
	...
	void LauncherLayer::CreateProject(std::filesystem::path projectPath)
	{
		...
		std::filesystem::copy(m_Properties.InstallPath + "/AntPlay/Resources/NewProjectTemplate", projectPath, std::filesystem::copy_options::recursive);
		...
		AddProjectToRecents(projectPath.string() + "/" + std::string(s_ProjectNameBuffer) + ".aproj");
	}
